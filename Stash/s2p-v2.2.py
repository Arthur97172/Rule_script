...
class SViewGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("S-View Created By Arthur Gu | V2.2")
    ...
    def reset_application(self):
        ...
        # 1. 调用 _initialize_state() 清空所有设置和数据
        self._initialize_state()
        
        # 2. 恢复/设置 Plot Type 和 Display Mode 为指定值
        # 确保 Plot Type 恢复到 "Magnitude (dB)"
        self.plot_type.set("Magnitude (dB)")
        # 确保 Display Mode 恢复到 "Normal"
        ...
        # 重新创建/更新依赖 Plot Type 的 UI (例如 limit mark tab)
        self.create_limit_mark_tab(self.plot_type.get()) 
        self.create_data_information_tab()
    ...
    def _handle_chart_output(self, copy=False):
        try:
            ...
            # ------------------- Normal 模式 -------------------
                ...
                YELLOW_FACECOLOR = (1.0, 1.0, 0.0, 0.9)
                ...
                for i, p in enumerate(self.params):
                    ...
                    for text in old_ax.texts:
                        try:
                            ...
                            ax_new.text(pos[0], pos[1], text.get_text(),
                                        ...
                                        ha=text.get_ha(), va=text.get_va(), bbox=bbox_style)
                        except Exception:
                            pass
