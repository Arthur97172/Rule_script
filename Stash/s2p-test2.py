...
# 文件: s2p-test1.py (在 SViewGUI 类内，替换 _setup_unified_y_control_ui 方法)
    ...
    def _find_custom_match_freq_hz(self, target_value, search_freq_hz, search_data_array, match_type):
        """
        在指定的频率和数据数组范围内，查找第一个或最后一个与目标值匹配的频率。
        使用线性插值找到精确的交叉点，如果没有交叉点则返回最接近点的频率。
        """
        
        # 1. 查找所有交叉点 (使用 np.diff(data < target) 查找交叉点索引)
        # is_below: True/False 数组，表示数据点是否低于目标值
        is_below = search_data_array < target_value
        # np.diff 找到 True->False 或 False->True 的变化
        cross_indices = np.where(np.diff(is_below))[0]

        crossing_freqs = []
        for i in cross_indices:
            # i 和 i+1 是交叉点所在的两个数据点的索引
            x1, x2 = search_freq_hz[i], search_freq_hz[i+1]
            y1, y2 = search_data_array[i], search_data_array[i+1]

            # 线性插值公式：x = x1 + (target - y1) * (x2 - x1) / (y2 - y1)
            if abs(y2 - y1) > 1e-9: # 避免除以零
                interpolated_freq = x1 + (target_value - y1) * (x2 - x1) / (y2 - y1)
                crossing_freqs.append(interpolated_freq)
        
        # 2. 如果找到精确交叉点，根据 First/Last Match 返回
        if crossing_freqs:
        ...
        # 3. 如果没有交叉点，返回最接近目标值的点的频率（作为 fallback）
        if len(search_data_array) > 0:
            closest_index = np.argmin(np.abs(search_data_array - target_value))
            return search_freq_hz[closest_index]
