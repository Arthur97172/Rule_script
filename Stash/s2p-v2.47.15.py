...
# ----------------------------------------------------
MARKER_SEARCH_TYPES = ["Max Value", "Min Value"]
...
class SViewGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("S-View Created By Arthur Gu | V2.47.15")
...
# 文件: s2p-test1.py (在 SViewGUI 类内，替换 _setup_unified_y_control_ui 方法)
                ...
                for mark in self.data[plot_type]["marks"].get(p, []):
                    try:
                        ...
                        if mark.get("is_search", False):
                            ...
                            # 初始化 x_display 为紧凑的范围格式（带 '@' 符号）
                            # 注意：我们在这里使用紧凑的 {start}-{stop}{unit} 格式
                            ...
                            if range_no_overlap:
                                ...
                                if np.any(mask):
                                    ...
                                    # 2. 【关键修改】将精确频率转换为 MHz 并格式化
                                    precise_f_mhz = precise_f_hz / 1e6 # 始终转为 MHz
                                    precise_f_str_mhz = f"{precise_f_mhz:.3f} MHz" # 格式化为 x.xxx MHz
                                    
                                    # 3. 更新 x_display 字符串为所需格式：@[Start]-[Stop][Unit]-[精确频率(MHz)]
                                    x_display = f"{start_str}-{stop_str}{unit}, {precise_f_str_mhz}"
                                    
                                    x_pt_original = precise_f_mhz # 直接使用 MHz 值
                                    y_pt = y_masked[idx]
                ...
                try:
                    ...
                    if mark.get("is_search", False):
                        ...
                        try:
                            ...
                            # 检查 Search Marker 的整个范围是否与数据集的范围完全无重叠
                            range_no_overlap = (f_max < min_f_hz) or (f_min > max_f_hz)
                            ...
                            if range_no_overlap:
                                ...
                                if np.any(mask):
                                    ...
                                    x_pt_plot = np.clip(x_pt_original, x_min_mhz, x_max_mhz) # 钳位
                                    y_str = f"{y_pt:.3f} {y_unit}"
                                    plot_marker_point = True
                                else:
                                    y_str = "N/A (No data in range)" # 搜索范围在数据范围内，但无数据点命中
                                    x_display = f"{start_str}-{stop_str}{unit} (No Data)" 

                        except ValueError:
                            y_str = "Invalid Freq"
                            x_display = f"{start_str}-{stop_str}{unit} (Invalid Input)"
    ...
    def _handle_chart_output(self, copy=False):
        try:
            ...
            # ------------------- Normal 模式 (核心修改) -------------------
                ...
                # --- 先从每个 on-screen fig 获取精确渲染（强制 draw 并按 PNG 导出） ---
                rendered_images = {}  # param -> PIL.Image
                for param in selected_params:
                    cfg = self.plot_configs.get(param)
                    if not cfg:
                        # 如果没有 on-screen fig，尝试用 plot_parameter_output 绘制到临时 fig（兜底）
                    ...
                    fig = cfg.get('fig')
                    # 强制 draw，以确保 interactive 状态与 blit 都被刷新到 Figure
                    try:
                        fig.canvas.draw()
                    except Exception:
                        # 有时 canvas 不是 Agg（兜底）
                ...
                # --- 根据 layout 将多个小图拼接到一个大图（像素级拼接，保持 on-screen 外观） ---
                # 这里以 out_fig canvas size = 1200x800 px 作为基准（可调整）
                OUT_W, OUT_H = 1200, 800
                canvas_img = Image.new('RGBA', (OUT_W, OUT_H), (255,255,255,255))

                # 计算每个格子区域（2x2 网格）
                # 当只有 1 个选中时，使用全屏；当 2 个选中并排时用上下两行（代码已在 _determine_normal_layout 里处理）
                # 我们通过 layout_config 的 row/col/rowspan/colspan 来决定放置位置
                cell_w = OUT_W // 2
                cell_h = OUT_H // 2
                for param, cfg in layout_config.items():
                    ...
                    # 等比缩放到 target 区域（可能会失真，但可保持整体布局）
                    img_resized = img.resize((target_w, target_h), Image.LANCZOS)
                    paste_x = col * cell_w
                    paste_y = row * cell_h
                    canvas_img.paste(img_resized, (paste_x, paste_y), img_resized)

                # 如果只有一张图，确保其居中或填满（上面逻辑也会填满全图）
                final_img = canvas_img.convert('RGB')  # 去掉 alpha
    ...
    def _draw_search_marker_frame(self, mark_data, container, plot_type, param):
        ...
        ttk.Combobox(frame, textvariable=mark_data["search_type"], values=MARKER_SEARCH_TYPES, state="readonly", width=10).grid(row=0, column=9, padx=3)
        ...
        status_btn.grid(row=0, column=10, padx=5)
        self._update_status_button_color(status_btn, mark_data["display_status"].get())
        mark_data["status_btn"] = status_btn  # 保存引用
        ...
        tk.Button(frame, text="Remove", bg="#e74c3c", fg="white", command=remove_and_update).grid(row=0, column=11, padx=5)
    # ----------------------------------------------------
    ...
    def update_file_list_ui(self):
        ...
        def _on_canvas_resize(event):
            # 修复后的代码：使用保存的 item ID 来配置窗口的宽度
            if hasattr(self, 'file_list_content_id'):
                # 确保宽度变化，并使用 configure 调整 Canvas item 的 width 属性
                self.file_list_canvas.itemconfig(self.file_list_content_id, width=event.width)

        # 绑定 Canvas 尺寸变化事件
        # 确保在每次更新时只绑定一次，或者确保 lambda 函数逻辑是您想要的
        self.file_list_canvas.unbind("<Configure>") # 先解绑，防止重复绑定
        ...
        for i, dataset in enumerate(self.datasets):
            ...
            label.pack(side="left", padx=5, fill="x", expand=True) # 使用 fill="x" 和 expand=True 让 label 占据大部分空间
            ...
            remove_btn.pack(side="right", padx=(5, 10), pady=0) # 靠右对齐
