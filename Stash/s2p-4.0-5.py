...
COLOR_CYCLE = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b', '#17becf', '#bcbd22', '#e377c2', '#7f7f7f', '#e41a1c', '#377eb8', '#4daf4a', '#984ea3', '#ff7f00', '#ffff33', '#a65628', '#f781bf', '#999999', '#66c2a5', '#a5cee3', '#fdbf6f', '#b2df8a', '#fb9a99', '#cab2d6', '#ffff99', '#b15928', '#ffffff', '#bdbdbd', '#6a3d9a','#ffbb78']
...
class SViewGUI:
    def __init__(self, root):
        ...
        self.default_params = ["S11", "S21", "S12", "S22"]
    ...
    def rebuild_show_params(self):
            ...
            inner = tk.Frame(self.cb_frame, bg="#f0f2f5")
            ...
            if mode == "Normal":
                ...
                for p in params:
                    if p not in self.show_param_vars:
                        # Normal 模式通常也需要默认选中 S11, S21
                        self.show_param_vars[p] = tk.BooleanVar(value=True)
                    # 确保 Normal 模式下 S 参数被选中
                ...
                # 2×2 布局
                grid = [["S11", "S21"], ["S12", "S22"]]

                for r,row in enumerate(grid):
                    for c,p in enumerate(row):
                        # 【Normal 模式也应添加 command=self.update_plots，保证刷新】
                        #chk = tk.Checkbutton(inner, text=p, variable=self.show_param_vars[p],
                        #                     bg="#f0f2f5", anchor="w", command=self.update_plots)                                               
                        chk = tk.Checkbutton(inner, text=p, variable=self.show_param_vars[p],
                                             bg="#f0f2f5", anchor="w")
            ...
            # 【Bug 2 修复：获取所有加载文件中的最大端口数 S4P > S3P > S2P】
            max_n_ports = 0
            # 遍历所有数据集，找到最大的 n_ports
            for ds in self.datasets:
                # 假设每个 dataset 字典中包含 "n_ports" 键，默认值为 2
            ...
            # 确保 BooleanVar 存在且默认选中 (修复 Bug 1 - Part 1: 默认选中)
            for p in params:
                if p not in self.show_param_vars:
                    # 如果是新的 S 参数，创建并设置为 True
                    self.show_param_vars[p] = tk.BooleanVar(value=True)
                
                # 强制设置为 True，实现加载 S3P/S4P 文件后默认全部选中
                self.show_param_vars[p].set(True)
 ...
 # Max mode添加Marker信息
    ...
    def update_plots(self):
        ...
        if mode == "Normal":
            ...
            # -------- 修复 Bug 1：强制 Normal 模式只使用 S11,S21,S12,S22 --------
            try:
                normal_params = self.default_params.copy()  # 通常为 ["S11","S21","S12","S22"]
            except Exception:
                normal_params = ["S11", "S21", "S12", "S22"]

            # 强制 self.params 为 4 个标准参数（保留顺序）
    ...
    def plot_combined(self, redraw_full=True):
        ...
        # 【修复】：防护初始化 max_show_param_vars（如果不存在）
        if not hasattr(self, 'max_show_param_vars'):
            self.max_show_param_vars = {}
        if plot_type not in self.max_show_param_vars:
            self.max_show_param_vars[plot_type] = {}
        show_vars = self.max_show_param_vars[plot_type]

        # 【新增】：收集所有可能参数（union of all datasets）
        ...
        # 初始化 show_vars for all params
        for param in all_possible_params:
            if param not in show_vars:
                show_vars[param] = tk.BooleanVar(value=True)

        # 【新增】：动态参数顺序（使用 get_s_param_order）
        dynamic_param_order = list(get_s_param_order(max_n_ports).keys())

        # 4. 绘制数据曲线
        # ==================== 终极修正：图例顺序改为 “先文件后参数” ====================
        # 关键修改：先遍历 dataset，再遍历参数 → 图例顺序完美！（动态参数）
        for dataset in self.datasets:
            data_id = dataset['id']
        ...
        ax.xaxis.set_minor_locator(AutoMinorLocator(2))
        ax.grid(True, which='minor', linestyle=':', linewidth=0.5, color='gray', alpha=0.5)
        # 8. Limit Lines – 动态循环所有possible params
        if plot_type in self.data and self.datasets:
            for p in all_possible_params:  # 使用动态all_possible_params
                if not show_vars[p].get():
                    continue
        ...
        if plot_type in self.data:
            for p in all_possible_params:  # 动态循环所有possible params
                if not show_vars[p].get():
                    continue
                # 普通 Marker
                for mark in self.data[plot_type]["marks"].get(p, []):
            ...
            def sort_key(info):
                ...
                if hasattr(self, 'get_marker_id_number'):
                    try:
                        ...
                        marker_num = 0
                # 终极排序键：先按 ID（加载顺序），再按参数顺序（动态S11→S21→...），再按 Marker 编号
    ...
    def _reindex_markers_and_refresh_ui(self, plot_type, param):
        ...
        # Clear regular marker frame
        for w in marker_list_frame.winfo_children():
            w.destroy()
        
        # Filter and reindex regular markers (is_search=False and is_avg=False)
        ...
        for mark in regular_marks:
            ...
            regular_count += 1
            self._draw_marker_frame(mark, marker_list_frame, plot_type, param)  # Existing method for regular markers
        
        # Clear and refresh search markers if UI ref exists
        if search_marker_list_frame:
            ...
            for mark in search_marks:
                if "frame" in mark and mark["frame"]:
                    mark["frame"].destroy()
                mark["id"] = f"S{search_count}"  # Separate ID prefix for search markers
                search_count += 1
                self._draw_search_marker_frame(mark, search_marker_list_frame, plot_type, param)  # Assume or implement this method
        
        # Clear and refresh average markers if UI ref exists
        if avg_list_frame:
            ...
            for mark in avg_marks:
                if "frame" in mark and mark["frame"]:
                    mark["frame"].destroy()
                mark["id"] = f"A{avg_count}"  # Separate ID prefix for average markers
                avg_count += 1
                self._draw_avg_marker_frame(mark, avg_list_frame, plot_type, param)  # Assume or implement this method
    ...
    def clear_all_datasets(self):
        ...
        # === 核心修复 1：销毁 Limits & Marks 标签页 UI 框架 ===
        ...
        # 3. UI 刷新和重绘 (在禁用绘图刷新的环境中进行)
        self.create_data_information_tab()
        self.create_axis_control_tab()
        self.update_data_information_tab() 
        
        # === 核心修复 2：重建 Limits & Marks 标签页 ===
        self.create_limit_mark_tab(self.plot_type.get())
        # =================================================
...
# ------------------------------------------------------------------
    ...
    def update_limit_mark_params(self):
        ...
        for param in params_to_use:
            ...
            # 调用创建参数部分的函数，确保每个 S 参数 Tab 的内容被正确初始化
    ...
    def create_limit_mark_section(self, parent, plot_type, param):
        ...
        def add_avg_item():
            if not self.datasets:
                messagebox.showwarning("Warning", "Please load an S2P file before adding an Average Value.")
                return
            data_id_options = [str(d['id']) for d in self.datasets]
            default_data_id = data_id_options[0] if data_id_options else "1"
            new_avg = {
