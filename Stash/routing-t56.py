...
class RoutingApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Routing Estimation Tool - V1.1.1")
    ...
    # --- 优化点：移除全局绑定，只对 results 区域进行局部绑定 ---
    ...
    def on_results_mousewheel(self, event):
        ...
        return "break" # 阻止事件传播，防止其他组件也滚动

    # --- 优化点：修复 Canvas 区域的滚轮处理函数，使用 yview 检查视图比例 ---
    def on_canvas_mousewheel(self, event, canvas):
        """处理标签页内 Canvas 的滚轮事件，并在不需滚动时不响应"""
        
        # 1. 获取 Canvas 的可视比例：(顶部分数, 底部分数)
        # 例如，(0.0, 1.0) 表示内容完全可见；(0.0, 0.5) 表示内容底部被切断一半。
        top_frac, bottom_frac = canvas.yview()
        
        # 2. [防止空滚] 检查内容是否已完全可见
        # 如果视图的范围 (bottom_frac - top_frac) 接近 1.0，说明内容未溢出，无需滚动。
        if abs(bottom_frac - top_frac) >= 0.999:
            return "break" # 阻止事件传播，实现无内容时不滚动

        # 3. 执行滚动操作
        delta = int(-1 * (event.delta / 120))
        
        # 4. 边界检查：如果 delta > 0 (向下滚) 且已经在底部，则阻止
        if delta > 0 and bottom_frac >= 0.999:
             return "break"
        
        # 5. 边界检查：如果 delta < 0 (向上滚) 且已经在顶部，则阻止
        if delta < 0 and top_frac <= 0.001:
             return "break"
    ...
    def create_tab(self, tab_name, ops_list):
        ...
        # ----------------------------------------------------
        # 定义统一的列配置 (宽度和对齐)
        # 修正：sticky 必须是 'n', 'e', 's', 'w' 的组合
        # ----------------------------------------------------
        # Col: 0        1          2       3        4            5               6                  7
        # Header: Select  Excluded  N° of P Qty  Std Time  Optimized %  Calc Time (min)  Operation
        
        # 定义每列的宽度 (W) 和对齐方式 (J: justify, S: sticky)
        # 'w' = 左对齐, 'e' = 右对齐, 'ew' = 填充并居中 (用于固定宽度的 Entry)
        COL_CONFIG = [
            (8, 'center', 'w'),   # 0: Select (Checkbutton 宽度, 靠左)
            (8, 'center', 'w'),   # 1: Excluded (Checkbutton 宽度, 靠左)
            (6, 'center', 'ew'),  # 2: N° of P (Entry 宽度, 填充并居中)
            (6, 'center', 'ew'),  # 3: Qty (Entry 宽度, 填充并居中)
            (12, 'center', 'w'),  # 4: Std Time (Entry(8)+Unit(4) 宽度, 靠左)
            (12, 'center', 'ew'),  # 5: Optimized % (Entry 宽度, 填充并居中)
            (15, 'center', 'w'),  # 6: Calc Time (min) (Label 宽度, 靠左)         
        ]
        ...
        header_frame.pack(fill='x', padx=1, pady=(0, 0)) 
        
        headers = ["Select", "Excluded", "N° of P", "Qty", "Std Time", "Optimized %", "Calc Time (min)", "Operation"]

        # 配置 header_frame 的列权重
        for col_idx in range(OP_COL):
            # 权重设为0，依靠固定宽度
            header_frame.grid_columnconfigure(col_idx, weight=0) 
        header_frame.grid_columnconfigure(OP_COL, weight=1) # Operation 列拉伸

        # 填充表头
        header_bg = '#d9d9d9'
        for col, text in enumerate(headers):
            lbl = tk.Label(header_frame, text=text, font=('Courier New', 9, 'bold'),
                             bg=header_bg, anchor='w', padx=4)
            
            sticky = 'ew' # 表头默认拉伸
            if col < OP_COL:
                # 强制设置宽度，确保头部 Label 和内容对齐
                lbl.config(width=COL_CONFIG[col][0]) 
                # 对于表头 Label，将 anchor 设置为 'center' 或 'w'
                lbl.config(anchor='center' if COL_CONFIG[col][2] == 'ew' else 'w') 
                # 使用 COL_CONFIG 中的 sticky 值
                sticky = COL_CONFIG[col][2]
            
            lbl.grid(row=0, column=col, padx=3, pady=5, sticky=sticky)
        ...
        # 配置 content_frame 的列权重，与 header_frame 保持一致
        for col_idx in range(OP_COL):
            content_frame.grid_columnconfigure(col_idx, weight=0) 
        content_frame.grid_columnconfigure(OP_COL, weight=1)
        ...
        for i, op in enumerate(ops_list, 1):
            ...
            # --- 列 0: Select (Checkbutton) ---
            ttk.Checkbutton(content_frame, variable=self.check_vars[tab_name][name]).grid(
                row=i, column=0, padx=25, pady=2, sticky=COL_CONFIG[0][2])
            
            # --- 列 1: Excluded (Checkbutton) ---
            ttk.Checkbutton(content_frame, variable=self.non_optimized_vars[tab_name][name]).grid(
                row=i, column=1, padx=25, pady=2, sticky=COL_CONFIG[1][2])
            
            # --- 列 2: N° of P (Entry) ---
            ttk.Entry(content_frame, textvariable=self.parts_qty_vars[tab_name][name],
                      width=COL_CONFIG[2][0], justify=COL_CONFIG[2][1]).grid(
                          row=i, column=2, padx=6, pady=2, sticky=COL_CONFIG[2][2])
            
            # --- 列 3: Qty (Entry) ---
            ttk.Entry(content_frame, textvariable=self.quantity_vars[tab_name][name],
                      width=COL_CONFIG[3][0], justify=COL_CONFIG[3][1]).grid(
                          row=i, column=3, padx=6, pady=2, sticky=COL_CONFIG[3][2])

            # --- 列 4: Std Time (Frame: Entry + Label) ---
            tf = ttk.Frame(content_frame)
            tf.grid(row=i, column=4, padx=3, pady=2, sticky=COL_CONFIG[4][2]) 
            ...
            # --- 列 5: Optimized % (Entry) ---
            ttk.Entry(content_frame, textvariable=self.opt_percent_vars[key],
                      width=COL_CONFIG[5][0], justify=COL_CONFIG[5][1]).grid(
                          row=i, column=5, padx=5, pady=2, sticky=COL_CONFIG[5][2])
            
            # --- 列 6: Calc time (Label) ---
            calc_lbl = tk.Label(content_frame, text="0.00",
                                font=('Courier New', 10),
                                fg='darkgreen',
                                bg=row_color, # 使用行颜色
                                width=COL_CONFIG[5][0],
                                anchor='w') # Label anchor 靠左对齐
            calc_lbl.grid(row=i, column=6, padx=8, pady=2, sticky=COL_CONFIG[6][2])
            self.calc_labels[tab_name][name] = calc_lbl

            # --- 列 7: Operation (Label) ---
            op_lbl = tk.Label(content_frame, text=name, anchor='w', justify='left',
                              wraplength=400, bg=row_color)
            op_lbl.grid(row=i, column=OP_COL, sticky='ew', padx=16, pady=2) # 使用 'ew' 确保拉伸
