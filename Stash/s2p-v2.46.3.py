...
class SViewGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("S-View Created By Arthur Gu | V2.46.3")
...
# 文件: s2p-test1.py (在 SViewGUI 类内，替换 _setup_unified_y_control_ui 方法)
    ...
    def on_scroll_zoom_combined(self, mpl_event):
        ...
        # 新增：更新 Y 轴刻度
        ax.relim()  # 重新计算轴限制
        ax.xaxis.set_major_locator(MaxNLocator(18))   ## 重新设置 X 轴 locator 以确保最多 18 个刻度
        #ax.autoscale_view(scaley=False)  # 自动缩放视图，但只针对 X 轴（Y 轴已手动设置）
        ax.yaxis.set_major_locator(MaxNLocator(10))  # 重新设置 Y 轴 locator 以确保最多 10 个刻度
    ...
    def plot_combined(self, redraw_full=True): 
        ...
        if unified_y_mode == "Custom":
            try:
                ...
                # 需要导入 MaxNLocator, MultipleLocator
                from matplotlib.ticker import MaxNLocator, MultipleLocator
    ...
    def _optimize_tick_labels_output(self, ax, fig):
        try:
            ...
            # === X 轴：动态调整（基于当前 xlim，确保包括边缘） ===
            xlim = ax.get_xlim()
            span = xlim[1] - xlim[0]
            
            # 动态决定小数位数（匹配带宽精度，如 0.5 MHz）
            if span < 1:
                decimals = 2  # 小范围：2 位小数 (e.g., 1993.25)
            elif span < 10:
                decimals = 1  # 中范围：1 位小数
            else:
                decimals = 0  # 大范围：整数
            
            # 使用 LinearLocator 强制包括 xlim 边缘，并均匀分布
            ax.xaxis.set_major_locator(ticker.LinearLocator(numticks=8))  # 包括边缘 + 中间
