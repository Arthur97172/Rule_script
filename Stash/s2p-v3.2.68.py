...
# ------------------------------------------------------------------
    ...
    def load_s2p(self):
        ...
        # ------------------------------------------------------------
        # 2. Batch Processing of File List (保持不变)
        # ------------------------------------------------------------
        loaded_count = 0
        failed_files = []
        ...
        try:
            ...
            if self.display_mode.get() == "Max" and self.max_fig:
                ...
                if current_height > default_height + 50: 
                    # 状态：最大化/调整大小（Logo 尺寸变大：225x90 改为180x72）
                    logo_w, logo_h = "180", "72"  # 优化后的新尺寸
            ...
            # ------------------- Normal 模式 -------------------
                ...
                self._draw_report_logo(out_fig, y_offset= 2, logo_width=normal_w, logo_height=normal_h) 
                # ------------------------------------------------------------------------
