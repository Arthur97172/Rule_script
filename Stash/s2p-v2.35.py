...
class SViewGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("S-View Created By Arthur Gu | V2.35")
    ...
    def setup_ui(self):
        ...
        # Serial Number改为Plot Details
        # 修改: 使用 LabelFrame 作为容器，将标签文字作为框架标题
        sn_group = tk.LabelFrame(control_stack_frame, text="Plot Details:", 
                ...
                try:
                    ...
                    if all_freq_values:
                        ...
                        if min_f_mhz >= max_f_mhz:
                            continue
                        if ltype == "Max":
                            ax.hlines(upper, min_f_mhz, max_f_mhz, colors='red', linestyles='--', linewidth=1.0, zorder=4)
                        else:
                            ax.hlines(lower, min_f_mhz, max_f_mhz, colors='green', linestyles='--', linewidth=1.0, zorder=4)
                except:
                    pass
    ...
    def create_data_information_tab(self):
        ...
        # 3. 创建自定义 ID 名称区域 (Customize File ID Name)
        # 此区域被移到文件列表下方，总结内容上方
        if not hasattr(self, 'custom_id_outer'):
            ...
            custom_id_frame = tk.LabelFrame(custom_id_outer, text="Customize File ID Name",
                                             font=("sans-serif", 10, "bold"), bg="#f0f2f5", labelanchor="nw")
