...
class SViewGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("S-View Created By Arthur Gu | V2.46.13")
...
# 文件: s2p-test1.py (在 SViewGUI 类内，替换 _setup_unified_y_control_ui 方法)
    ...
    def create_data_information_tab(self):
        ...
        if not hasattr(self, 'customize_color_frame'):
            ...
            # 1. Label and Combobox for Data ID
            tk.Label(color_input_frame, text="Data ID:", bg="#f0f2f5").pack(side="left", padx=(0, 5))
            ...
            tk.Label(color_input_frame, text="Current Color:", bg="#f0f2f5").pack(side="left", padx=(10, 5))
            ...
            tk.Button(color_input_frame, text="Select Color", 
                      command=self._open_color_picker).pack(side="left", padx=5)
            
            # 4. Apply & Clear Buttons
            tk.Button(color_input_frame, text="Apply Color", 
                      command=self._apply_custom_color).pack(side="left", padx=5)
            tk.Button(color_input_frame, text="Clear Colors", bg="#e74c3c", fg="white", 
                      command=self._clear_custom_colors).pack(side="left", padx=5)
        # ----------------------------------------------------------------------
    ...
    def _clear_custom_colors(self):
        """
        [优化] 强制清除所有 Data ID 的自定义颜色，并清空颜色/重命名下拉菜单的选中记录和名称输入框。
        执行完成后弹出成功提示。
        """
        ...
        if hasattr(self, 'selected_data_id_var'):
            self.selected_data_id_var.set("") # 清空名称自定义下拉菜单的选中记录
        if hasattr(self, 'custom_name_var'):
            self.custom_name_var.set("") # 清空自定义名称输入框
