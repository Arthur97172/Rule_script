...
#2.46.50移除自定义ID颜色
#2.46.51修复Normal模式下Marker标记拖拽的bug
#2.46.52修复Normal模式下第一次添加Marker是出现重复Marker的bug
#2.46.53修复Normal模式下拖动图表出现残影的bug
#2.46.54修复disable_refresh拖动Marker Legend图表恢复大小的bug
#2.46.55修复拖动Marker Legend图表闪烁2下的bug
#2.46.56增加自定义Marker Legend颜色自定义功能
#2.47添加Peak Marker Search功能
#2.47.1修复Limits & Marks标签页的bug
#2.47.2添加Marker超出范围显示Out of Freq Range提示
#2.47.3修复添加Marker标签后再次加载文件无法识别新文件Ref ID的bug
#2.47.4添加无文件时禁止添加Marker和Limit Line
#2.47.5优化Peak Marker Search功能
#2.47.6修复Normal模式下无法删除Peak Search Marker标签的bug
#2.47.7优化Max模式下未加载文件时的显示
#2.47.8优化Limits & Markers输入框大小
...
class SViewGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("S-View Created By Arthur Gu | V2.47.8")
...
# 文件: s2p-test1.py (在 SViewGUI 类内，替换 _setup_unified_y_control_ui 方法)
    ...
    def _draw_marker_frame(self, mark_data, container, plot_type, param):
        ...
        # Freq
        tk.Label(frame, text="Freq:", bg="#ffffff").grid(row=0, column=1, padx=3)
    ...
    def create_data_information_tab(self):
        ...
        if not hasattr(self, 'summary_content_frame'):
            ...
            # 注意：这里的 pady 从 15 改为 (0, 15)，因为上面的 custom_id_outer/marker_legend_outer 已经提供了顶部边距
    ...
    def _create_marker_legend_config_ui(self, master_frame):
        ...
        Args:
            master_frame: 主容器 Frame (data_information_tab)。
        Returns:
            marker_legend_frame: 创建的外层 Frame，用于定位。
        """
        
        # --- 外层 Frame (对应 Custom ID Frame 的外层 frame) ---
        marker_legend_outer = tk.Frame(master_frame, bg="#f0f2f5")
        # 放在 Custom ID 区域的下方，使用 pack，设置 y 轴边距，并左对齐
        marker_legend_outer.pack(fill="x", side="top", anchor="w", padx=15, pady=(0, 10)) # 调整 pady 确保间距
        
        marker_legend_labelframe = tk.LabelFrame(
            marker_legend_outer, 
            text="Marker Legend Customization",
            font=("sans-serif", 10), 
            bg="#f0f2f5", 
            labelanchor="nw"
        )
        ...
        # 绑定事件：任何改变都触发图表刷新
        # 使用 self.root.after(100, self.update_plots) 可减少快速点击时的闪烁
        #移除自动刷新
        #self.marker_legend_configs["boxstyle_var"].trace_add("write", lambda *args: self.update_plots())
        #self.marker_legend_configs["facecolor_var"].trace_add("write", lambda *args: self.update_plots())
        #self.marker_legend_configs["alpha_var"].trace_add("write", lambda *args: self.update_plots())
