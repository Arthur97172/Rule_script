...
class SViewGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("S-View Created By Arthur Gu | V4.0.7")
 ...
 # Max mode添加Marker信息
    ...
    def plot_combined(self, visible_params=None):
        ...
        if self.max_fig:
            ...
            self.max_fig.subplots_adjust(top=0.98, bottom=0.08, left=0.055, right=0.98)
    ...
    def plot_parameter_output(self, ax, fig, param, plot_type):
        ...
        if y_mode != "Custom" and all_y_values:
            ...
            if s_param_index in [0,3]: group_params = ["S11","S22"]
            elif s_param_index in [1,2]: group_params = ["S12","S21"]
            else: group_params=[param]

            grouped=[]
            for dataset in self.datasets:
                for p in group_params:
                    y_data=self._get_y_data_for_limit_calc(dataset,p,plot_type)
                    if len(y_data)>0: grouped.append(y_data)

            if grouped:
                y_all = np.concatenate(grouped)
                y_min,y_max = np.min(y_all),np.max(y_all)
                #y_min,y_max,y_step=self._calculate_friendly_y_limits(y_min,y_max)
