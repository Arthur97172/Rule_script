...
class SViewGUI:
    ...
    def _update_normal_params(self, *args):
        ...
        # 【关键修复】：在 Normal 模式下，将主参数列表 self.params 限制为这 4 个。
        # 这样可以确保 self.update_plots 只迭代当前选中的 4 个参数。
        self.params = new_params 
        
        # 强制更新 Normal 模式下绘图和 Checkbutton 所依赖的参数列表
        self.default_params = new_params
        
        # 确保新的 S 参数有对应的绘图配置 (S13, S31, S33, etc.)
        self._ensure_plot_configs_exist(new_params) 
        
        # 重新构建 UI 面板和图表
        self.rebuild_show_params()
        
        # 🔥🔥🔥【关键新增】端口切换 → 自动刷新 Limits & Marks tabs
    ...
    def _rebuild_y_axis_tabs(self):
        ...
        不新增结构，不新增配置，只复用 create_axis_control_tab 内的逻辑。
        """
    ...
    def _refresh_y_axis_tabs(self):
        ...
        仅在 refresh 时触发，不在 Port Pair 切换时触发。
        """
        # Y Axis 控件还没创建，不操作
        if not hasattr(self, "y_sub_notebook"):
            return

        # 当前 Normal 模式应该显示 self.params 对应的 S 参数
    ...
    def rebuild_show_params(self):
        ...
        if mode == "Normal":
            ...
            # 绑定事件：当选择变化时，调用 _update_normal_params (会触发两次，但可以接受)
            self.port_selection_var.trace_add("write", self._update_normal_params)
            ...
            for p in params:
                if p not in self.show_param_vars:
                    # Normal 模式通常也需要默认选中 S11, S21 或对应的 S_pp, S_qp
            ...
            for r, row in enumerate(grid):
                for c, p in enumerate(row):
                    # 保持原有的 command=self.update_plots
                    chk = tk.Checkbutton(
                        cb_inner_frame, 
                        text=p, 
 ...
 # Max mode添加Marker信息
    ...
    def update_plots(self):
        ...
        if mode == "Normal":
            ...
            try:
                # 使用 self.default_params, 它已经被 _update_normal_params 更新为选中的端口对
                normal_params = self.default_params.copy() 
            except Exception:
                # 容错处理
                normal_params = ["S11", "S12", "S21", "S22"] 
                
            # 强制 self.params 为 4 个标准参数（保留顺序）
            self.params = normal_params # <--- 核心修改，使用新的参数列表
    ...
    def plot_parameter_output(self, ax, fig, param, plot_type):
        ...
        for dataset in self.datasets:
            ...
            if plot_type == "Magnitude (dB)":
                ...
                y_data, freq_mhz_gd = self.calculate_group_delay(freq, s_data)
...
# ------------------------------------------------------------------
    ...
    def update_limit_mark_params(self):
        ...
        # ------------------------------------------------------------------
        # 2. 根据显示模式决定应该显示哪些 S 参数
        # ------------------------------------------------------------------
        mode = self.display_mode.get()  # "Normal" or "Max"

        if mode == "Normal":
            # 获取 Show Params 内选择的端口对（例如 "2-3"）
            try:
            ...
            # Max 模式：显示所有真实存在的 S 参数
            params_to_use = sorted(self.data[plot_type]["limit_lines"].keys())
    ...
    def update_marker_position_ui(self):
        """
        Normal 模式：根据 Port Pair 更新 Normal 的 4 个标签页（Sxx, Sxy, Syx, Syy）
        Max 模式：不改变，只显示全局控件
        """
        if not hasattr(self, "normal_position_notebook"):
            return
