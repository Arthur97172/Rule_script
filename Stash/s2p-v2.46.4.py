...
class SViewGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("S-View Created By Arthur Gu | V2.46.4")
    ...
    def _initialize_state(self):
        ...
        self.DRAG_UPDATE_INTERVAL = 30    # 更新间隔 (毫秒)。推荐 30-50ms
    ...
    def setup_ui(self):
        ...
        left_margin = 20
        # 父容器布局
        control_stack_frame.pack(fill="x", padx=5, pady=5)
        ...
        tk.Button(file_ops_group, text="Clear Data", font=("sans-serif", 10, "bold"),
                  bg="#e74c3c", fg="white", relief="flat", anchor="w",
                  padx=left_margin, pady=6, command=self.clear_all_datasets)\
            .pack(fill="x", padx=5, pady=2)

        tk.Button(file_ops_group, text="Reset App", font=("sans-serif", 10, "bold"),
                  bg="#3F51B5", fg="white", relief="flat", anchor="w",
                  padx=left_margin, pady=6, command=self.reset_application)\
            .pack(fill="x", padx=5, pady=2)
...
# 文件: s2p-test1.py (在 SViewGUI 类内，替换 _setup_unified_y_control_ui 方法)
    ...
    def on_scroll_zoom_combined(self, mpl_event):
        ...
        ax.relim()  # 重新计算轴限制
        #ax.autoscale_view(scaley=False)  # 自动缩放视图，但只针对 X 轴（Y 轴已手动设置）
        ax.yaxis.set_major_locator(MaxNLocator(15))  # 重新设置 Y 轴 locator 以确保最多 10 个刻度
