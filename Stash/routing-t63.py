...
5. Qty 默认根据 N° Housings 自动更新（用户修改后保持不变）
...
#V1.0最初版
#V1.0.1鼠标滚轮滚动取消全局控制，把标签页部分和计算部分的控制分开
#V1.0.2输出的excel表格紧缺到小数点2位
#V1.0.3代工厂默认设置为GLOBAL
#V1.0.4 Tuning tab: All Qty default to N° Housings (× default_qty=1)
#V1.0.5 Custom FPY: Input only number, % fixed on right
...
class RoutingApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Routing Estimation Tool - V1.0.5")
        ...
        # [V1.0.5] Separate num var for custom input (only number)
        self.standard_custom_fpy_num_var = tk.StringVar(value='88')
        self.standard_rework_var = tk.StringVar(value='12%')
        self.optimized_fpy_var = tk.StringVar(value='88%')
        # [V1.0.5] Separate num var for custom input (only number)
        self.optimized_custom_fpy_num_var = tk.StringVar(value='88')
        ...
        for tab in ['Pre_Assembly', 'Assembly', 'Tuning', 'Testing', 'Quality Check']:
            ...
            for name in self.quantity_vars[tab]:
                if not self.user_modified.get(tab, {}).get(name, False):
                    ...
                    # All items in these tabs now default to default_qty * housing (no special case for Tuning non-RF)
    ...
    def on_product_change(self, *args):
        ...
        if prod in FPY:
            ...
            # [V1.0.5] Set num var to number only
    ...
    def on_standard_fpy_combo_change(self, *args):
        if self.standard_fpy_var.get() == 'Custom':
            ...
            # [V1.0.5] Sync num var if switching from preset
            if self.standard_fpy_var.get() != 'Custom':  # Wait, no: when selecting Custom, set num to current preset's number
            ...
            # Sync num var to current preset
            self.standard_custom_fpy_num_var.set(self.standard_fpy_var.get().strip('%'))
    ...
    def on_optimized_fpy_combo_change(self, *args):
        if self.optimized_fpy_var.get() == 'Custom':
            ...
            # [V1.0.5] Sync num var if switching from preset
            if self.optimized_fpy_var.get() != 'Custom':  # Similar
    ...
    def update_standard_fpy_rework(self, *args):
        try:
            if self.standard_fpy_var.get() == 'Custom':
                # [V1.0.5] Use num var directly (it's number only)
    ...
    def reset_all(self):
        ...
        # [V1.0.5] Reset num var
        self.standard_custom_fpy_num_var.set('88')
        self.standard_rework_var.set('12%')
        self.optimized_fpy_var.set('88%')
        # [V1.0.5] Reset num var
        self.optimized_custom_fpy_num_var.set('88')
        self.optimized_rework_var.set('12%')
        housing = self.get_housing()
        ...
        for tab in self.check_vars:
            ...
            for name, var in self.quantity_vars[tab].items():
                for op in section_ops[tab]:
                    if op[0] == name:
                        default_qty = op[3]
                        # All items in Pre_Assembly/Assembly/Tuning/Testing/Quality Check now reset to default_qty * housing (no special case for Tuning non-RF)
                        if tab in ['Pre_Assembly', 'Assembly', 'Tuning', 'Testing', 'Quality Check']:
                ...
                for name, std_t_default, opt_percent_default_int, _, unit, _ in ops:
                    ...
                    if self.check_vars[tab][name].get():
                        ...
                        rows.append({
                            ...
                            "Calc Time STANDARD (min)": round(minutes_std, 2),
