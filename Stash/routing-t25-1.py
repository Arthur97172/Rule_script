...
"""
    ...
    def export_to_excel(self):
        ...
        try:
            ...
            for tab, ops in [('Pre_Assembly', PRE_ASSEMBLY_OPS), ('Assembly', ASSEMBLY_OPS), ('Tuning', TUNING_OPS),
                ...
                for name, _, opt_t, _, unit, _ in ops:
                    if self.check_vars[tab][name].get():
                        parts_qty_val = self.safe_get_int(self.parts_qty_vars[tab][name]) if tab in self.parts_qty_vars else 1
                        rows.append({
                            "Section": tab,
                            "Operation": name,
                            "Parts Qty": parts_qty_val,
                            "Qty": self.safe_get_int(self.quantity_vars[tab][name]),
                            "Std Time": f"{self.safe_get_double((tab,name)):.2f} {unit}",
            ...
            if tuning_type in TUNING_TYPES and n_cav > 0:
                std_sec, opt_sec = TUNING_TYPES[tuning_type]
                rf_tuning_time_base = ((std_sec if mode == 'STANDARD' else opt_sec) / 60.0) * n_cav * housing
                rf_tuning_time = rf_tuning_time_base * factor
            ...
            with pd.ExcelWriter(filename, engine='openpyxl') as writer:
                ...
                summary_data = [
                    ["Pre_Assembly", round(pre_assembly_base,3), factor, round(pre_assembly,3)],
                    ["Assembly", round(assembly_base,3), factor, round(assembly,3)],
                    ["RF Tuning", round(rf_tuning_time_base,3), factor, round(rf_tuning_time,3)],
                    ["Tuning", round(tuning_base,3), factor, round(tuning,3)],
                    ["Testing", round(testing_base,3), factor, round(testing,3)],
