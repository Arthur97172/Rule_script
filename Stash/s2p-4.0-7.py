...
class SViewGUI:
    ...
    def rebuild_show_params(self):
        ...
        for p in params:
            ...
            # 【核心修复：移除强制设置】：不再强制set(True)，保留用户当前状态
            # self.show_param_vars[p].set(True)  # ← 删除或注释此行，实现加载 S3P/S4P 文件后默认全部选中，但用户可修改
 ...
 # Max mode添加Marker信息
    ...
    def _determine_normal_layout(self):
        ...
        if num_selected == 4:
            ...
            layout = {
                ...
                "S12": {'row': 0, 'col': 1, 'rowspan': 1, 'colspan': 1},
                "S21": {'row': 1, 'col': 0, 'rowspan': 1, 'colspan': 1},
            ...
            if unselected in ["S11", "S22"]:
                # ✅ 只取消 S11 或 S22 (S12, S21 上部1x1; S22/S11 下部1x2)
                p_lower_full = "S22" if unselected == "S11" else "S11"
                
                layout["S12"] = {'row': 0, 'col': 0, 'rowspan': 1, 'colspan': 1}
                layout["S21"] = {'row': 0, 'col': 1, 'rowspan': 1, 'colspan': 1}
                layout[p_lower_full] = {'row': 1, 'col': 0, 'rowspan': 1, 'colspan': 2}
                
            elif unselected in ["12", "S21"]:
                # ✅ 只取消 S12 或 S21 (S11, S22 上部1x1; S21/S12 下部1x2)
                p_lower_full = "S21" if unselected == "S12" else "S12"
            ...
            # ✅ 取消两个参数 (上部1x2 + 下部1x2，按优先级 S11 > S12 > S21 > S22)
            priority_map = {'S11': 0, 'S12': 1, 'S21': 2, 'S22': 3}
            # 按照优先级升序排列 (0最高)
    ...
    def update_plots(self):
        ...
        if mode == "Normal":
            ...
            # -------- 修复 Bug 1：强制 Normal 模式只使用 S11,S12,S21,S22 --------
            try:
                normal_params = self.default_params.copy()  # 通常为 ["S11","S12","S21","S22"]
            except Exception:
