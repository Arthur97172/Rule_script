...
PRE_ASSEMBLY_OPS = [
    ...
    ("Stripline + bais tee or coil soldered", 15, 100, 1, 'sec', 'housing'),    
...
SUB_ASSEMBLY_OPS = [
    ...
    ("Heat shrink tubing", 8, 100, 1, 'sec', 'housing'),
    ("LNA (or shield cover) screwing", 3, 100, 1, 'sec', 'housing'),
    ...
    ("Low pass support fixing", 5, 100, 1, 'sec', 'housing'), 
    ("Label for s/n", 5, 100, 1, 'sec', 'housing'),
    ("Label for port", 5, 100, 1, 'sec', 'housing'),
    ("Label for LED Light", 10, 100, 1, 'sec', 'housing'),    
    ("PWBA installation (including soldering)", 8, 100, 1, 'sec', 'housing'),
    ("Placing PWBA Shield Cover", 3, 100, 1, 'sec', 'housing'),
    ("Ret connector", 20, 100, 1, 'sec', 'housing'),
    ("Resonator", 7, 90, 1, 'sec', 'housing'),
    ("Tuning lid cover screwing", 3, 85, 1, 'sec', 'housing'),
    ("Temperature Aging", 1.0, 100, 1, 'min', 'housing'),        
    ("Other Sub Assembly item", 5.0, 100, 1, 'min', 'housing'),
]

FINAL_ASSEMBLY_OPS = [
    ("Gasket out cover", 45, 100, 1, 'sec', 'housing'),
    ("Ground screw + nut + washer - Check Qty", 45, 100, 1, 'sec', 'housing'),  
    ...
    ("Label for unit", 5, 100, 1, 'sec', 'housing'),
    ("Outer cover (time x screw)", 4, 100, 1, 'sec', 'housing'),
    ("PWBA cable routing", 10, 100, 1, 'sec', 'housing'),
    ("Other Final Assembly item", 5.0, 100, 1, 'min', 'housing'),
]

CLEANING_OPS = [
    ("Cleaning Tuning lid open and close (time x screw)", 4, 95, 1, 'sec', 'housing'),
    ("Cleaning Process", 4.0, 100, 1, 'min', 'housing'),
    ("Other Cleaning item", 5.0, 100, 1, 'min', 'housing'),
]
...
TESTING_OPS = [
    ("Auto LED TEST", 2.0, 100, 1, 'min', 'none'),
    ("AC/DC or Generic Measures", 3.0, 100, 1, 'min', 'none'),
    ("IP3 (optional for TMA)", 3.0, 100, 1, 'min', 'none'),
    ("IMD Fixed Tone Test", 1.5, 100, 1, 'min', 'none'),
    ("IMD Swept Tone Test", 1.25, 100, 1, 'min', 'none'),
...
class RoutingApp:
    def __init__(self, root):
        ...
        self.complexity_sections = ['Pre_Assembly', 'Sub_Assembly', 'Tuning', 'Cleaning', 'Testing', 'Final_Assembly', 'Quality Check', 'Packaging']
    ...
    def setup_data(self):
        sections = {
            ...
            'Sub_Assembly': SUB_ASSEMBLY_OPS,
            'Tuning': TUNING_OPS,
            'Cleaning': CLEANING_OPS,
            'Testing': TESTING_OPS,
            'Final_Assembly': FINAL_ASSEMBLY_OPS,
        ...
        for tab, ops in sections.items():
            ...
            for op in ops:
                ...
                def mark_std_time_user_modified(*args, t=tab, n=name):
                    if self.suppress_updates:
                        return
                    self.user_modified.setdefault(t, {})[n] = True

                # ✅ Std Time 只绑定这一个 trace
                self.std_time_vars[key].trace_add(
                    'write',
                    lambda *args, t=tab, n=name: (
                        mark_std_time_user_modified(*args, t=t, n=n),
                        self._trace_update_section(t)
                    )
                )
    ...
    def setup_ui(self):
        ...
        # ========== 新增：伸缩功能支持 ==========
        def add_collapsible_section(parent_frame, title_text):
            header_frame = ttk.Frame(parent_frame)
            header_frame.pack(fill='x')

            # ------------------------ 关键修改 ------------------------
            # 1. 移除 width=25，使按钮宽度自适应内容
            toggle_btn = ttk.Button(header_frame, text=f"{title_text}  ▼")
            # 2. 添加 padx=10，将按钮推离左侧边缘，使其靠左对齐但不紧贴边框
            toggle_btn.pack(side='left', anchor='w', padx=10) 
            # --------------------------------------------------------

            # 内容框架
            # 增加 padding=(10, 5, 10, 10) 模拟 LabelFrame 的内部边距，使内容对齐
            content_frame = ttk.Frame(parent_frame, padding=(10, 5, 10, 10))
            content_frame.pack(fill='x')

            def toggle():
                if content_frame.winfo_viewable():
                    content_frame.pack_forget()
                    toggle_btn.config(text=f"{title_text}  ▲")
                else:
                    content_frame.pack(fill='x')
                    toggle_btn.config(text=f"{title_text}  ▼")
            ...
            # ---- 关键：记录起来供 reset_all 恢复用 ----
            self._collapsible_sections.append((title_text, toggle_btn, content_frame))

            return content_frame, toggle_btn

        # ========== Basic Configuration"（可收缩） ==========
        basic_outer = ttk.LabelFrame(self.main_frame, text="")
        basic_outer.pack(fill='x', pady=(0, 1))
        basic_frame, _ = add_collapsible_section(basic_outer, "Basic Configuration")
        ...
        # ========== Yield Estimation（可收缩） ==========
        run_outer = ttk.LabelFrame(self.main_frame, text="")
        run_outer.pack(fill='x', pady=(0, 1))
        run_frame, _ = add_collapsible_section(run_outer, "Yield Estimation")
        ...
        # ========== Complexity Level（可收缩） ==========
        comp_outer = ttk.LabelFrame(self.main_frame, text="")
        comp_outer.pack(fill='x', pady=(0, 1))
        complexity_frame, _ = add_collapsible_section(comp_outer, "Complexity Level")
        ...
        for i in range(6, len(sections)):
            ...
            col = (i - 6) * 2
            ttk.Label(complexity_frame, text=f"{section}:", width=label_width, anchor='e').grid(
                row=2, column=col, sticky='e', padx=(2, 6), pady=(8, 0))
            comp_frame = ttk.Frame(complexity_frame)
            comp_frame.grid(row=2, column=col+1, sticky='w', padx=(0, 10), pady=(8, 0))
            ttk.Combobox(comp_frame, textvariable=self.complexity_vars[section],
                         values=COMPLEXITY_LEVELS, width=7, state='readonly').pack(side='left')
            setattr(self, f"{section.lower()}_complexity_entry", ttk.Entry(comp_frame,
                                                                         textvariable=self.custom_complexity_num_vars[section],
        ...
        # ========== Routing Parameters（可收缩） ==========
        param_outer = ttk.LabelFrame(self.main_frame, text="")
        param_outer.pack(fill='x', pady=(0, 5))
        param_frame, _ = add_collapsible_section(param_outer, "Routing Parameters")
        ...
        for tab_name, ops_list in [('Pre_Assembly', PRE_ASSEMBLY_OPS), ('Sub_Assembly', SUB_ASSEMBLY_OPS),
                              ('Tuning', TUNING_OPS), ('Cleaning', CLEANING_OPS), ('Testing', TESTING_OPS), ('Final_Assembly', FINAL_ASSEMBLY_OPS),
    ...
    def update_section_times(self, tab_name):
        ...
        try:
            ops_list = {
                'Pre_Assembly': PRE_ASSEMBLY_OPS, 'Sub_Assembly': SUB_ASSEMBLY_OPS,
                'Tuning': TUNING_OPS, 'Cleaning': CLEANING_OPS, 'Testing': TESTING_OPS, 'Final_Assembly': FINAL_ASSEMBLY_OPS,
                'Quality Check': QUALITY_OPS, 'Packaging': PACKAGING_OPS
            }[tab_name]
    ...
    def reset_all(self):
        ...
        # ---- 在恢复 Qty 之前必须先清空 user_modified（关键修复）----
        for tab in ['Pre_Assembly', 'Sub_Assembly', 'Tuning', 'Cleaning', 'Testing', 'Final_Assembly', 'Quality Check']:
            self.user_modified[tab] = {}
        ...
        try:
            for tab in self.check_vars:
                for name in self.check_vars[tab]:
                    ...
                    # 恢复优化百分比
                    self.opt_percent_vars[key].set(
                        next(
                            (op[2] for op in (
                                PRE_ASSEMBLY_OPS + SUB_ASSEMBLY_OPS + TUNING_OPS + CLEANING_OPS +
                                TESTING_OPS + FINAL_ASSEMBLY_OPS + QUALITY_OPS + PACKAGING_OPS
                            ) if op[0] == name),
                            100
                        )
                    )
        ...
        # ---- 恢复所有折叠区（Yield Estimation + Complexity Level）为展开状态 ----
        for title, btn, frame in self._collapsible_sections:
    ...
    def calculate_detailed(self):
        ...
        for mode, fpy in selected_modes:
            ...
            sub_assembly_base = self.get_section_total('Sub_Assembly', SUB_ASSEMBLY_OPS, mode)
            cleaning_base = self.get_section_total('Cleaning', CLEANING_OPS, mode)
            final_assembly_base = self.get_section_total('Final_Assembly', FINAL_ASSEMBLY_OPS, mode)
            assembly_base = sub_assembly_base + cleaning_base + final_assembly_base
            ...
            complexity_a = self.get_complexity_level('Sub_Assembly')
            complexity_c = self.get_complexity_level('Cleaning')
            complexity_t = self.get_complexity_level('Tuning')
            complexity_te = self.get_complexity_level('Testing')
            complexity_fa = self.get_complexity_level('Final_Assembly')
...
Complexity Levels  :
Pre_Assembly   : {complexity_p:<8.2f}Sub_Assembly   : {complexity_a:<8.2f}
Cleaning       : {complexity_c:<8.2f}Tuning         : {complexity_t:<8.2f}
Testing        : {complexity_te:<8.2f}Final_Assembly : {complexity_fa:<8.2f}
...
Cleaning           : {cleaning_base:6.2f}  (Included in Assembly)
...
"""
    ...
    def export_to_excel(self):
        ...
        try:
            ...
            rows = []
            for tab, ops in [('Pre_Assembly', PRE_ASSEMBLY_OPS), ('Sub_Assembly', SUB_ASSEMBLY_OPS), ('Tuning', TUNING_OPS),
                            ('Cleaning', CLEANING_OPS), ('Testing', TESTING_OPS), ('Final_Assembly', FINAL_ASSEMBLY_OPS), ('Quality Check', QUALITY_OPS), ('Packaging', PACKAGING_OPS)]:
                ...
                for name, std_t_default, opt_percent_default_int, _, unit, mtype in ops:
                    ...
                    if self.check_vars[tab][name].get():
                        ...
                        minutes_std = float(self.calc_labels[tab][name].cget("text").strip())

                        # Calc Time OPTIMIZED (min)
                        if self.non_optimized_vars[tab][name].get():
                            minutes_opt = 0.0
                        else:
                            minutes_opt = minutes_std * (opt_percent / 100.0)
            ...
            with pd.ExcelWriter(filename, engine='openpyxl') as writer:
                config_data = [
                    ...
                    ["Complexity Factor (Sub_Assembly)", f"{self.get_complexity_level('Sub_Assembly'):.2f}"],
                    ["Complexity Factor (Tuning)", f"{self.get_complexity_level('Tuning'):.2f}"],
                    ["Complexity Factor (Cleaning)", f"{self.get_complexity_level('Cleaning'):.2f}"],
                    ["Complexity Factor (Testing)", f"{self.get_complexity_level('Testing'):.2f}"],
                    ["Complexity Factor (Final_Assembly)", f"{self.get_complexity_level('Final_Assembly'):.2f}"],
                ...
                #Time Summary不显示Cleaning
                #items = ["Pre_Assembly", "Assembly", "Cleaning", "Tuning", "Testing", "Quality Check", "Packaging", "Rework", "Total w/o Rework", "Total Routing Time"]
    ...
    def import_from_excel(self):
        ...
        required_conf_keys = [
            ...
            "Complexity Factor (Pre_Assembly)", "Complexity Factor (Sub_Assembly)",
            "Complexity Factor (Tuning)", "Complexity Factor (Cleaning)", "Complexity Factor (Testing)",
            "Complexity Factor (Final_Assembly)", "Complexity Factor (Quality Check)", "Complexity Factor (Packaging)"
        ]
        ...
        valid_sections = {
            ...
            'Sub_Assembly': [op[0] for op in SUB_ASSEMBLY_OPS],
            'Tuning': [op[0] for op in TUNING_OPS],
            'Cleaning': [op[0] for op in CLEANING_OPS],
            'Testing': [op[0] for op in TESTING_OPS],
            'Final_Assembly': [op[0] for op in FINAL_ASSEMBLY_OPS],
        ...
        try:
            ...
            restore_complexity_for_section("Complexity Factor (Sub_Assembly)", self.complexity_vars['Sub_Assembly'], self.custom_complexity_num_vars['Sub_Assembly'])
            ...
            restore_complexity_for_section("Complexity Factor (Cleaning)", self.complexity_vars['Cleaning'], self.custom_complexity_num_vars['Cleaning'])
            ...
            restore_complexity_for_section("Complexity Factor (Final_Assembly)", self.complexity_vars['Final_Assembly'], self.custom_complexity_num_vars['Final_Assembly'])
            ...
            for tab in self.check_vars:
                for name in list(self.check_vars[tab].keys()):
                    ...
                    if key in self.original_std_times:
                        self.std_time_vars[key].set(self.original_std_times[key])
                        self.opt_percent_vars[key].set(next((op[2] for op in (PRE_ASSEMBLY_OPS+SUB_ASSEMBLY_OPS+TUNING_OPS+CLEANING_OPS+TESTING_OPS+FINAL_ASSEMBLY_OPS+QUALITY_OPS+PACKAGING_OPS) if op[0]==name), 100))
            ...
            for entry in ops_to_apply:
                ...
                default_qty = None
                for ops in (PRE_ASSEMBLY_OPS, SUB_ASSEMBLY_OPS, TUNING_OPS, CLEANING_OPS, TESTING_OPS, FINAL_ASSEMBLY_OPS, QUALITY_OPS, PACKAGING_OPS):
                    for op in ops:
                        if op[0] == operation:
                            default_qty = op[3]
                            break
                ...
                if default_qty is None:
                    ...
                    if section in ['Pre_Assembly', 'Sub_Assembly', 'Tuning', 'Cleaning', 'Testing', 'Final_Assembly', 'Quality Check']:
    ...
    def on_qty_change(self, tab, name, *args):
        if self.suppress_qty_modify:
            return
        if tab in ['Pre_Assembly', 'Sub_Assembly', 'Tuning', 'Cleaning', 'Testing', 'Final_Assembly', 'Quality Check']:
