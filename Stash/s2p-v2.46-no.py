...
class SViewGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("S-View Created By Arthur Gu | V2.46")
    ...
    def _initialize_state(self):
        ...
        self.DRAG_UPDATE_INTERVAL = 50    # 更新间隔 (毫秒)。推荐 30-50ms
    ...
    def _safe_refresh_markers(self, reset_limits=True):
        ...
        从而在刷新 Marker 标记的同时保持图表的缩放/平移状态，并修复 Y 轴刻度不协调的问题。
        """
        ...
        # 3. 如果不需要重置，且 Limits 成功保存，则恢复 Limits 并修复刻度
        if not reset_limits:
            try:
                if self.display_mode.get() == "Normal":
                    for p, limits in current_limits.items():
                        if p in self.plot_configs:
                            ...
                            # [核心修复] 恢复 Limits 后，强制使用 MaxNLocator 重新计算合理的刻度
                            # MaxNLocator(nbins=10) 确保 Y 轴刻度最多为 10 个，且间隔合理
                    ...
                    if self.max_ax and "max" in current_limits:
                        ...
                        # [核心修复] 恢复 Limits 后，强制使用 MaxNLocator 重新计算合理的刻度
                        self.max_ax.yaxis.set_major_locator(MaxNLocator(nbins=10))
    ...
    def _draw_limit_check_status(self, ax, check_results):
        ...
        Args:
            ...
            check_results (list): 包含 {'name': str, 'status': 'PASS'|'FAIL'|'N/A_NoLimits'|'N/A_NoData'} 的列表。
        """
        ...
        if not self.datasets or all(res['status'] in ['N/A_NoLimits', 'N/A_NoData'] for res in check_results) or not has_limit_check_results:
            # 如果没有数据集，或者所有文件状态都是 N/A，或者整个图表没有限制线
            status_text = "N/A"
            color = 'gray'
        ...
        # ha='right' 锚定文本框的右边缘。multialignment='left' 确保文本在框内左对齐。
    ...
    def setup_ui(self):
        ...
        tk.Checkbutton(inner_frame,
                       ...
                       bg="#f0f2f5",
                       anchor='w',  # 左对齐复选框和文本
                       justify='left').pack(anchor='w', padx=(5, 0), pady=0)  # 左侧5px缩进，右侧无填充
    ...
    def create_axis_control_tab(self):
        ...
        tk.Radiobutton(x_frame, text="Default", variable=self.axis_configs["x_mode"], value="Default", bg="#f0f2f5").pack(anchor="w", padx=10)
        tk.Radiobutton(x_frame, text="Custom", variable=self.axis_configs["x_mode"], value="Custom", bg="#f0f2f5").pack(anchor="w", padx=10)

        custom_x_frame = tk.Frame(x_frame, bg="#f0f2f5")
        custom_x_frame.pack(fill="x", padx=20, pady=5)
        ...
        # 移除了Start后面的频率单位下拉菜单
        #ttk.Combobox(custom_x_frame, textvariable=self.axis_configs["x_unit"], values=["MHz", "GHz"], state="readonly", width=6).pack(side="left", padx=5)
        ...
        # Y Axis 控制 (per plot_type sub-notebook)
        y_frame = tk.LabelFrame(self.axis_control_tab, text="Y Axis Control (per Plot Type & Parameter)", font=("sans-serif", 11, "bold"), bg="#f0f2f5")
        y_frame.pack(fill="both", expand=True, padx=15, pady=10)

        self.y_sub_notebook = ttk.Notebook(y_frame)
        self.y_sub_notebook.pack(fill="both", expand=True)
        # --- 新增：绑定事件以禁用手动切换 ---
        # 当标签页尝试切换时，调用 self._prevent_y_axis_notebook_switch
        ...
        for pt in SUPPORTED_PLOT_TYPES:
            ...
            for p in self.params:
                ...
                y_mode_var = self.y_configs[pt][p]["mode"]
                tk.Radiobutton(p_y_frame, text="Default", variable=y_mode_var, value="Default", bg="#f0f2f5").pack(anchor="w", padx=10)
                tk.Radiobutton(p_y_frame, text="Custom", variable=y_mode_var, value="Custom", bg="#f0f2f5").pack(anchor="w", padx=10)

                custom_y_frame = tk.Frame(p_y_frame, bg="#f0f2f5")
                custom_y_frame.pack(fill="x", padx=20, pady=5)

                tk.Label(custom_y_frame, text="Max:", bg="#f0f2f5").pack(side="left", padx=(0, 5))
                tk.Entry(custom_y_frame, textvariable=self.y_configs[pt][p]["max"], width=10).pack(side="left", padx=5)

                tk.Label(custom_y_frame, text="Min:", bg="#f0f2f5").pack(side="left", padx=(20, 0))
                tk.Entry(custom_y_frame, textvariable=self.y_configs[pt][p]["min"], width=10).pack(side="left", padx=5)
    ...
    def on_scroll_zoom_normal(self, mpl_event, param):
        ...
        # 新增：更新 Y 轴刻度
        ax.relim()  # 重新计算轴限制
        ax.autoscale_view(scaley=False)  # 自动缩放视图，但只针对 X 轴（Y 轴已手动设置）
        ax.yaxis.set_major_locator(MaxNLocator(10))  # 重新设置 Y 轴 locator 以确保最多 10 个刻度
    ...
    # 在 on_scroll_zoom_combined 函数中，类似添加刻度更新逻辑
    ...
    def add_marker_on_click_combined(self, mpl_event):
        ...
        if not self.marker_click_enabled.get(): # <--- 新增检查
            return
    ...
    def add_marker_on_click_normal(self, event, param):
        ...
        for dataset in self.datasets:
            ...
            if plot_type == "Magnitude (dB)":
                ...
                try:
                    # 确保 self.calculate_group_delay 函数在类中存在
                    data_array, _ = self.calculate_group_delay(freq, s_data) 
                except:
                    continue 
        ...
        if not self.disable_refresh_var.get():
            # 允许刷新：执行完整刷新
            self.update_plots()
            self.status_var.set(f"Marker added at {f_val:g} {f_unit} for {param} on Data ID {target_data_id}.")
        else:
            # 关闭刷新：执行安全刷新 (保持 Limits)
            self._safe_refresh_markers(reset_limits=False)
            self.status_var.set(f"Marker added at {f_val:g} {f_unit} for {param} on Data ID {target_data_id}. Zoom state preserved.")
    ...
    # 新增辅助函数 (放在 on_marker_legend_motion 后面)
    ...
    def delete_marker_on_right_click(self, mpl_event, param=None):
        """处理鼠标右键点击事件，删除最近的 Marker，支持 Magnitude, Phase, 和 Group Delay 的 Y 轴容忍度检查。"""
            ...
            for mark in marks_list:
                ...
                if abs(click_mhz - marker_freq_mhz) <= tolerance_mhz:
                    ...
                    if marker_value is not None:
                        # 如果成功计算了 Y 值，则要求满足 Y 轴容忍度
                        if isinstance(marker_value, (int, float)) and abs(click_value - marker_value) <= Y_TOLERANCE_UNITS:
                            is_hit = True
                    else:
        ...
        if hit_marker:
            ...
            if not self.disable_refresh_var.get():
                ...
                # 关闭刷新：执行安全刷新 (保持 Limits)
                # 假设您已将 _safe_refresh_markers 添加到类中
                self._safe_refresh_markers(reset_limits=False)
    ...
    def on_display_mode_change(self, *args):
        ...
        # 控制 Cursor Coordinates 显示（两种模式都显示，但顺序要在底部）
    ...
    def _draw_max_plot(self):
        """
        绘制 Max Mode 下的组合 S 参数图。
        将所有数据集的所有 S 参数绘制在同一个坐标轴上。
        """
        if not self.max_ax or not self.datasets:
            return
        ...
        # 1. 清空图表并设置标题/标签
        ax.clear()
        ax.set_title(f"Combined S-Parameters ({plot_type})", fontsize=12, fontweight='bold')
        ...
        lines_artists = {} # 用于存储 line artists 以便创建 legend
        
        # 2. 遍历所有数据集和所有参数，并绘制数据
        for dataset in self.datasets:
            data_id = dataset['id']
            freq = dataset['freq']
            ...
            for param in self.params: # self.params = ["S11", "S21", "S12", "S22"]
                s = dataset['s_data'].get(param.lower())
                if s is None or len(s) == 0:
                    continue

                line_label = f"ID {data_id} - {param}"
                ...
                if plot_type == "Magnitude (dB)":
                    ...
                    # Group Delay 需要专门处理频率
                    y_data, freq_mhz_gd = self.calculate_group_delay(freq, s)
                    freq_mhz_plot = freq_mhz_gd
                else:
                    continue
                
                if plot_type != "Group Delay (ns)":
                    freq_mhz_plot = freq_mhz

                # 绘制线条 (linewidth=1.0)
                line, = ax.plot(freq_mhz_plot, y_data, color=color, linewidth=1.0, label=line_label)
                all_y_values.extend(y_data)
                all_freq_values.extend(freq_mhz_plot)
                lines_artists[line_label] = line
        ...
        if all_y_values:
            ...
            # Max 模式下使用全局 Y 轴配置
            y_mode = self.axis_configs["y_mode"].get() 
            
            if y_mode == "Auto":
                y_min, y_max, y_step = self._calculate_friendly_y_limits(y_min_data, y_max_data)
            elif y_mode == "Custom":
                try:
                    y_min = float(self.axis_configs["y_start"].get())
                    y_max = float(self.axis_configs["y_stop"].get())
                    y_step = (y_max - y_min) / 10 # 简单计算一个步长
                except ValueError:
                    # 回退到自动
                    y_min, y_max, y_step = self._calculate_friendly_y_limits(y_min_data, y_max_data)
        ...
        # 5. 绘制 Markers (所有参数的 Markers 都绘制在 Max 图上)
                ...
                for mark in self.data[plot_type]["marks"][param]:
                    try:
                        ...
                        for dataset in self.datasets:
                            ...
                            # Max 模式 Marker 信息 (包含参数名)
                            custom_name = self.custom_id_names.get(str(data_id), f"ID {data_id}")    
                            marker_legend_text.append(f"{mark_id} ({custom_name} - {param}) @{x_display}, {val:.3f} {y_unit}")
                            
                    except Exception:
                        # 确保异常不会导致整个图表崩溃
                        pass
        
        # 6. 绘制 Marker Legend Box 并存储 Artist (Max Mode)
        if marker_legend_text:
            # 读取 Max mode 配置 (来自 self.max_marker_pos_configs[plot_type])
            ...
            if mode == "Top Left":
                ...
                try:
                    # 注意：Max 模式的 Custom 坐标来自 self.max_marker_pos_configs
            ...
            # 绘制 Marker Legend 并存储 Artist 对象用于拖动
            ...
            self.max_marker_legend_artists[plot_type] = legend_artist
        else:
        ...
        # 只显示 ID + Param 的组合标签
        if lines_artists:
            ax.legend(lines_artists.values(), lines_artists.keys(), loc='lower right', fontsize=8, fancybox=True, shadow=False)
            
        # 8. 绘制 Limit Lines (Max 模式下绘制所有 S 参数的 Limit Lines)
        # 请注意：Max 模式下 Limit Line 的行为可能需要仔细定义，这里仅提供 Normal 模式的 Limit Line 逻辑聚合
        if plot_type in self.data and self.datasets:
            if all_freq_values:
                ...
                for param in self.params:
                    for line in self.data[plot_type]["limit_lines"][param]:
                        try:
                            ...
                            # 取 Limit Line 的频率范围与实际数据的频率范围的交集
                            plot_min_f_mhz = max(min_f_mhz, start_mhz)
                            plot_max_f_mhz = min(max_f_mhz, stop_mhz)
                            
                            if plot_min_f_mhz >= plot_max_f_mhz:
                                continue
                                
                            # Limit line 从虚线改为实线 (linewidth=1.0)
                            if ltype == "Max":
                                ax.hlines(upper, plot_min_f_mhz, plot_max_f_mhz, colors='blue', linestyles='-', linewidth=1.0, zorder=4)
                            else:
                                ax.hlines(lower, plot_min_f_mhz, plot_max_f_mhz, colors='blue', linestyles='-', linewidth=1.0, zorder=4)
                        except:
                            pass
                            
        # 9. 调用 canvas.draw()
        # 假设 Max canvas 是 self.max_canvas
        self.max_canvas.draw_idle()
    ...
    def plot_combined(self):
                        ...
                        try:
                            ...
                            if np.any(freq_mask):
                                ...
                                if violation:
                                    ...
                                    break # 发现 FAIL，立即退出当前 Limit Line 检查
                                    
                        except Exception:
                            pass
                    
                    # 检查完成后，设置后缀
                    pass_fail_suffix = f" ({pass_fail_status})"
        ...
        is_custom_y = False
        if visible_params:
            y_mode = self.y_configs[plot_type][visible_params[0]]["mode"].get()
            
            if y_mode == "Custom":
                try:
                    y_min_custom = float(self.y_configs[plot_type][visible_params[0]]["min"].get())
                    y_max_custom = float(self.y_configs[plot_type][visible_params[0]]["max"].get())
        ...
        # 8. Limit lines drawing for Combined Plot 
        if plot_type in self.data and self.datasets:
            # 仅在全局 Limit Check 开启时才绘制限制线
    ...
    def plot_parameter_output(self, ax, fig, param, plot_type):
            ...
            for mark in self.data[plot_type]["marks"][param]:
                try:
                    ...
                    for dataset in self.datasets:
                        ...
                        custom_name = self.custom_id_names.get(str(data_id), f"ID {data_id}")  
                        # Marker Legend 保持显示原始频率值 x_display
                        
                        # --- [修改 2]：收集 Marker ID, Data ID 和完整文本 (已修正) ---
                        full_legend_text = f"{mark_id} ({custom_name}) @{x_display}, {val:.3f} {y_unit}"
                        # 存储: (Marker ID str, Data ID str, Full Legend Text)
                        marker_info_list.append((mark_id, selected_data_id, full_legend_text))
                        # -------------------------------------------
        ...
        # --- [修改 3]：在绘制前添加 Normal Mode 排序逻辑 (已修正) ---
        if marker_info_list:
            ...
            # 2. 执行排序
            sorted_markers = sorted(marker_info_list, key=normal_mode_sort_key)
            ...
            # -------------------------------------------------------------
            # 【核心修改】绘制 Marker Legend：增加 multialignment='left'
            legend_artist = ax.text(
                x_val, y_val, txt, transform=ax.transAxes, fontsize=9, 
                verticalalignment=v_align, horizontalalignment=h_align, 
                multialignment='left',  # <--- 核心修改：确保多行文本在框内左对齐
                bbox=dict(boxstyle="round,pad=0.3", facecolor="yellow", alpha=0.9), 
                zorder=7
            )
            ...
            # 【新增】如果没有 Marker Legend，则清除 Artist 引用
            self.normal_marker_legend_artists[param] = None

        # ----------------------------------------------------
        # [新增功能] Limits Check Status (Normal Mode)
        # ----------------------------------------------------
        # 假设 plot_parameter_output 是 Normal Mode 的绘图函数
            ...
            if not limit_lines:
                ...
                for dataset in self.datasets:
                    ...
                    if s_data is None or len(s_data) < 2:
                        # 无数据
                        status = 'N/A_NoData'
                    elif self._check_dataset_limits(dataset, plot_type, param):
                        status = 'PASS'
                    else:
                        status = 'FAIL'
        ...
        ax.xaxis.set_major_locator(MaxNLocator(10))
        # ax.yaxis.set_major_locator(MaxNLocator(10)) # 已被新的 MultipleLocator 逻辑取代或覆盖
    ...
    def _optimize_tick_labels_output(self, ax, fig):
            try:
                ...
                def smart_y_formatter(y):
                    """
                    如果数字接近整数，显示为整数 (e.g., 10)。
                    否则，显示为两位小数 (e.g., 10.15)。
                    """
                    # 使用小的容忍度 (1e-6) 检查是否接近整数
                ...
                new_x = [lbl.get_text() for lbl in x_labels]
    ...
    def create_limit_mark_section(self, parent, plot_type, param):
        ...
        def draw_limit_line_frame(line_data, container):
            ...
            # 交换 Max 和 Min 的显示顺序
            ttk.Combobox(frame, textvariable=type_var, values=["Min", "Max"], width=6, state="readonly").grid(row=0, column=1, padx=3)
