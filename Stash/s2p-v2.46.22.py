...
#2.46.21增加自定义Marker标记自定义颜色
#2.46.22修复Marker位置显示问题。
...
class SViewGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("S-View Created By Arthur Gu | V2.46.22")
    ...
    def _initialize_state(self):
        ...
        # 【MODIFIED】 存储 Marker Legend 的 Matplotlib Text Artist 对象，用于点击检测
        # 原：self.normal_marker_legend_artists = {p: None for p in self.params} 
        # 修改为存储艺术家列表，以便在每个 Normal 子图上存储多个文本 Artist
        self.normal_marker_legend_artists = {p: [] for p in self.params} # <<< MOD: 存储艺术家列表
        self.max_marker_legend_artists = {} # 将按 plot_type 存储列表 {plot_type: list_of_artists}
...
# 文件: s2p-test1.py (在 SViewGUI 类内，替换 _setup_unified_y_control_ui 方法)
    ...
    def on_marker_legend_press(self, mpl_event):
        ...
        # --- 1. Marker 命中检测 (MODIFIED LOGIC) ---

        # 1.1. Max Mode 命中检测
        if is_max_mode:
            # MOD: 迭代 Max 模式的 Artist 列表
            if plot_type in self.max_marker_legend_artists:
                for artist in self.max_marker_legend_artists[plot_type]: 
        ...
        # 1.2. Normal Mode 命中检测
            ...
            # MOD: 迭代 Normal 模式的 Artist 列表
            for p, artists_list in self.normal_marker_legend_artists.items(): 
                for artist in artists_list: 
                    if artist and artist.contains(mpl_event)[0] and artist.axes == mpl_event.inaxes:
                        ...
                        break # 找到即退出
                if hit_artist: # 如果在某个 param 中找到了，则不再检查其他 param
                    break
    ...
    def plot_combined(self, redraw_full=True):
        ...
        # 确保 Matplotlib 导入
        from matplotlib.ticker import MaxNLocator, MultipleLocator, AutoMinorLocator
        from matplotlib.patches import Rectangle # 确保 Rectangle 可用
            ...
            for p in visible_params:
                for line in self.data[plot_type]["limit_lines"].get(p, []):
                    try:
                        ...
                        max_f_mhz = min(x_max_mhz, max(start_mhz, stop_val))
                ...
                for mark in self.data[plot_type]["marks"].get(p, []):
                    try:
                        ...
                        if status_val == "Display":
                            # (Marker ID str, Parameter str, Full Legend Text, Data ID str)
        ...
        # ----------------------------------------------------
        # 11. 【MAX 模式 Marker Legend 绘制 - 动态宽度与边界校正】
        # ----------------------------------------------------
        ...
        if visible_marker_info_list:
            ...
            # --- Marker Legend 核心参数 ---
            LINE_HEIGHT_NORM = 0.030 
            HORIZONTAL_PADDING_NORM = 0.005 
            W_RECT_NORM = 0.42 # 归一化宽度 (需根据最长文本调整)
            SAFETY_MARGIN_NORM = 0.03 # 确保不溢出的安全余量
            
            # ==========================================================
            # === 【动态宽度计算：V3】 ===
            # ==========================================================
            W_RECT_NORM = 0.30 # Fallback
            if self.max_fig and self.max_fig.canvas:
                renderer = self.max_fig.canvas.get_renderer()
                max_text_width_px = 0
                legend_texts = [info[2] for info in sorted_markers]
                font_size_pt = 9 

                for text in legend_texts:
                    # 预渲染一个临时文本对象来测量
                    temp_text = ax.text(0, 0, text, fontsize=font_size_pt, transform=ax.transAxes)
                    bbox = temp_text.get_window_extent(renderer=renderer)
                    width_px = bbox.width
                    max_text_width_px = max(max_text_width_px, width_px)
                    temp_text.remove()

                fig_width_px = self.max_fig.get_size_inches()[0] * self.max_fig.dpi
                
                text_width_norm = max_text_width_px / fig_width_px
                
                W_RECT_DYNAMIC = text_width_norm + 2 * HORIZONTAL_PADDING_NORM + SAFETY_MARGIN_NORM 
                W_RECT_NORM = max(W_RECT_DYNAMIC, 0.15) # 确保最小宽度
            # ==========================================================

            # 绘制 Marker Legend 文本框 - 位置配置
            pos_config = self.max_marker_pos_configs[plot_type]
            mode = pos_config["mode_var"].get()
            x_val, y_val = 0.98, 0.98
            h_align, v_align = 'right', 'top' # 整个Legend框的锚定点
            ...
            if mode == "Top Left": x_val, y_val, h_align, v_align = 0.02, 0.98, 'left', 'top'
                ...
                try:
                    x_val = float(pos_config["x_var"].get())
                    y_val = float(pos_config["y_var"].get())
                    h_align = pos_config["h_align_var"].get() 
                    v_align = pos_config["v_align_var"].get()
                except:
                    x_val, y_val = 0.98, 0.98
            ...
            if v_align == 'top':
                start_y_top = y_val 
                y_step = -LINE_HEIGHT_NORM
            elif v_align == 'bottom':
                start_y_bottom = y_val
                y_step = LINE_HEIGHT_NORM
            else: # 'center'/'custom'
                total_height = num_lines * LINE_HEIGHT_NORM
                start_y_bottom = y_val - total_height / 2 
                y_step = LINE_HEIGHT_NORM
            
            # --- X 轴起始点计算：计算出 x_rect (矩形的左侧 X 坐标) ---
            if h_align == 'right':
                x_rect = x_val - W_RECT_NORM 
            elif h_align == 'left':
                x_rect = x_val 
            else: # 'center'
                x_rect = x_val - W_RECT_NORM / 2
                
            x_text = x_rect + HORIZONTAL_PADDING_NORM 
            h_text_align = 'left' 
            
            # 5. 绘制整个盒子的外部边框 - 计算高度和底部 Y 坐标
            total_height_rect = num_lines * LINE_HEIGHT_NORM
            
            if v_align == 'top':
                 outer_box_y = start_y_top - total_height_rect
            elif v_align == 'bottom':
                outer_box_y = start_y_bottom
            else: 
                outer_box_y = start_y_bottom 
            ...
            if x_rect < 0:
                x_rect = 0.0
            elif x_rect + W_RECT_NORM > 1.0:
                x_rect = 1.0 - W_RECT_NORM
                
            # 重新计算文本起始点 (因为 x_rect 可能已改变)
            x_text = x_rect + HORIZONTAL_PADDING_NORM
            
            # 2. Y 轴校正
            if outer_box_y < 0:
                outer_box_y = 0.0
            elif outer_box_y + total_height_rect > 1.0:
                outer_box_y = 1.0 - total_height_rect

            # ==========================================================
                
            # 5. 绘制整个盒子的外部边框 (使用校正后的坐标，无边框透明背景)
            ...
            self.max_marker_artists.append(outer_rect) 


            # 6. 绘制内部色块和文本 (使用校正后的坐标，透明背景)
            for i, marker_info in enumerate(sorted_markers):
                marker_id, p_param, legend_text, data_id_str = marker_info
                
                # 确定背景颜色 (仅用于文本，背景已设为透明)
                try:
                    data_id_int = int(data_id_str)
                    bg_color = self.custom_id_colors.get(data_id_int, 'white')  
                except:
                    bg_color = 'white'

                # --- 背景矩形绘制 (Patch) - 使用校正后的 outer_box_y 来计算 rect_y_bottom ---
                if v_align == 'top':
                    rect_y_bottom = outer_box_y + total_height_rect - LINE_HEIGHT_NORM * (i + 1)
                elif v_align == 'bottom':
                    rect_y_bottom = outer_box_y + LINE_HEIGHT_NORM * i
                else: # 'center'
                    rect_y_bottom = outer_box_y + LINE_HEIGHT_NORM * i

                rect_patch = Rectangle(
                    (x_rect, rect_y_bottom), 
                    W_RECT_NORM, 
                    LINE_HEIGHT_NORM, 
                    transform=ax.transAxes,
                    # 【透明背景设置】
                ...
                self.max_marker_artists.append(rect_patch) 

                # --- 文本叠加绘制 (无 BBOX) ---
                text_y_center = rect_y_bottom + LINE_HEIGHT_NORM / 2.0
                
                txt_artist = ax.text(x_text, text_y_center, legend_text,
                                            transform=ax.transAxes,
                                            fontsize=9,
                                            color='black', 
                                            verticalalignment='center', 
                                            horizontalalignment=h_text_align,
                                            zorder=7) 
    ...
    def plot_parameter_output(self, ax, fig, param, plot_type):
        ...
        if not self.show_param_vars[param].get():
            ...
            # 【MODIFIED】：清除旧的 Legend Artists 列表中的所有引用，并重置列表
                ...
                try:
                    ...
                    for dataset in self.datasets:
                        ...
                        if display_status == "Display":
                            # 存储: (Marker ID str, Parameter str, Full Legend Text, Data ID str)
                            visible_marker_info_list.append((mark_id, param, full_legend_text, selected_data_id))
                        # -------------------------------------------
        ...
        # --- [最终优化]: 强制左对齐 + 确保色块填充整行 + 增加行间距 ---
        
        # 1. 清除旧的 Legend Artists 列表中的所有引用，并重置列表 (确保在绘制新内容前彻底清除)
        if param in self.normal_marker_legend_artists:
            for artist in self.normal_marker_legend_artists[param]:
                try:
                    artist.remove()
                    ...
                    pass
            self.normal_marker_legend_artists[param] = [] # 重置列表

        if visible_marker_info_list:
            
            # 定义 Normal Mode 排序键（Data ID -> Marker ID）
            ...
            # ** 核心参数 **
            LINE_HEIGHT_NORM = 0.045 # 【修正】增加行高，改善视觉拥挤问题
            HORIZONTAL_PADDING_NORM = 0.005
            W_RECT_NORM = 0.53 # 归一化宽度 (需根据最长文本调整)
            SAFETY_MARGIN_NORM = 0.05 # 确保不溢出的安全余量
            ...
            # --- X 轴起始点计算：实现内部左对齐 ---
            if h_align == 'right':
                x_rect = x_val - W_RECT_NORM # 矩形的左侧 X 坐标
            elif h_align == 'left':
                x_rect = x_val # 矩形的左侧 X 坐标
            else: # 'center'
                x_rect = x_val - W_RECT_NORM / 2
                
            x_text = x_rect + HORIZONTAL_PADDING_NORM  
            h_text_align = 'left' # 文本对齐方式 (强制左对齐)
            ...
            # 需要确保导入了 matplotlib.patches.Rectangle
            outer_rect = Rectangle(
                (x_rect, outer_box_y),  
                W_RECT_NORM,  
                total_height_rect,
                transform=ax.transAxes,
                facecolor='none',  
                alpha=0.0, # 即使 facecolor 是 'none'，alpha=0 也可以作为额外的安全措施
                # 【核心修改点：将 edgecolor 从 'black' 改为 'none' Marker显示区域的边框】
                edgecolor='none',  
                linewidth=1.0,
                zorder=6  
            )
            ax.add_patch(outer_rect)
            self.normal_marker_legend_artists[param].append(outer_rect)
            ...
            for i, marker_info in enumerate(sorted_markers):
                ...
                # --- 背景矩形绘制 (Patch) ---
                if v_align == 'top':
                    rect_y_bottom = start_y_top + y_step * (i + 1)  
                elif v_align == 'bottom':
                    rect_y_bottom = start_y_bottom + y_step * i
                else:  
                    rect_y_bottom = start_y_bottom + y_step * i
                ...
                ax.add_patch(rect_patch)
                self.normal_marker_legend_artists[param].append(rect_patch)  
                ...
                self.normal_marker_legend_artists[param].append(txt_artist)  
