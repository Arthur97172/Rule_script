...
class SViewGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("S-View Created By Arthur Gu | V2.46.Test")
...
# 文件: s2p-test1.py (在 SViewGUI 类内，替换 _setup_unified_y_control_ui 方法)
	...
	# 9. 绘制 Marker (实现 Display/Hide 逻辑)
        ...
        if hasattr(self, 'max_marker_artists'):
            for artist in self.max_marker_artists:
                artist.remove() # 移除旧的艺术家
            self.max_marker_artists = [] # 清空列表
        else:
            self.max_marker_artists = [] # 确保列表存在
             ...
             for p in visible_params:
                 for mark in self.data[plot_type]["marks"].get(p, []):
                     try:
                         ...
                         # Draw marker
                         # 无论是 Display 还是 Hide，Marker 标记 (点和M1文字) 都应该绘制
                         marker_artist = ax.plot(x_pt_plot, y_pt, '.', markerfacecolor='none', markeredgecolor=color, 
                                                 markersize=4, markeredgewidth=2, zorder=5)
                         ...
                         # 收集 Marker Artists (Max 模式下，M1 标记始终绘制)
                         self.max_marker_artists.extend(marker_artist)
                         self.max_marker_artists.append(annotation_artist)

                         # 收集 Marker 信息 (只有当 display_status 为 "Display" 时才收集 Legend 文本)
                         if display_status == "Display":
                             full_legend_text = f"{mark['id']} ({p} {custom_name}) @{x_display}, {val:.3f} {y_unit}"
                             # 存储: (Marker ID str, Param str, Full Legend Text, Data ID str)
                             visible_marker_info_list.append((mark['id'], p, full_legend_text, selected_data_id))
                             
                     except Exception:
                         pass

        # 10. Matplotlib Legend for Data Lines (保持不变)
        ax.legend(loc='best', fontsize=9, framealpha=0.7)
        
        # 11. Max Mode Marker Legend 排序和显示 (使用 visible_marker_info_list)
        txt_artist = None
        # --- 【修改】使用 visible_marker_info_list ---
        if visible_marker_info_list: 
             
            PARAM_ORDER = {'S11': 0, 'S21': 1, 'S12': 2, 'S22': 3}
             
            def max_mode_sort_key(info):
                 # info = (Marker ID str, Param str, Full Legend Text, Data ID str)
                 param_str = info[1]
                 data_id_str = info[3]
                 ...
                 # 加上 Marker ID 编号作为次级排序键
                 marker_num = self.get_marker_id_number(info[0]) 
                 return (data_id_number, param_index, marker_num) # 三重排序
 
            sorted_markers = sorted(visible_marker_info_list, key=max_mode_sort_key)
            # 提取第三个元素 (Full Legend Text)
            marker_legend_text = [info[2] for info in sorted_markers] 
            txt = "\n".join(marker_legend_text)
    ...
    def _draw_marker_frame_and_bind(self, mark_data, plot_type, param, marker_list_frame, canvas):
        ...
        # ------------------ UI 布局 (Grid Column Index: 0 | 1 | 2 | 3 | 4 | 5 | 6) ------------------
        # Col 0: Marker ID (M1:) - 修复：直接使用 mark_id，因为它是一个字符串
        ...
        # Col 5: 新增 Display/Hide 切换按钮
        tk.Button(frame, 
                  textvariable=display_var, 
                  command=toggle_display_status, # 保留：用户主动点击，需要刷新
                  bg="#5DADE2", fg="white", relief="flat", 
                  width=6).grid(row=0, column=5, padx=(5, 0), sticky="ew")

        # Col 6: Remove 按钮
        tk.Button(frame, text="Remove", bg="#95a5a6", fg="white", command=remove_and_update).grid(row=0, column=6, padx=5)
