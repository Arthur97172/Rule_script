...
class SViewGUI:
    ...
    def setup_ui(self):
        ...
        # --- 新增 Reset 按钮 ---
        tk.Button(top_frame, text="Reset", font=("sans-serif", 12, "bold"),
                  bg="#3F51B5", fg="white", relief="flat", padx=20, pady=8,
                  command=self.reset_application).pack(side="left", padx=5)
        # ----------------------
    ...
    def _draw_marker_frame_and_bind(self, mark_data, plot_type, param, marker_list_frame, canvas):
        ...
        for var in [freq_var, unit_var, data_id_var]:
            # 需要清除旧的 trace，但由于每次重排时都会销毁并重建 UI，
            # 这里的 trace_add 会在每次绘制时生效，确保实时更新。
        ...
        if plot_type in self.data and self.datasets:
            for mark in self.data[plot_type]["marks"][param]:
                try:
                    ...
                    unit = mark["unit"].get()
                    f = float(f_str)
                    freq_val = f * 1e9 if unit == "GHz" else f * 1e6 # 转换成 Hz
