...
class SViewGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("S-View Created By Arthur Gu | V2.46.18")
...
# 文件: s2p-test1.py (在 SViewGUI 类内，替换 _setup_unified_y_control_ui 方法)
    ...
    def _on_mouse_move_custom(self, event, param_or_combined):
        ...
        # === 核心修正：整合光标坐标显示逻辑 ===
        # =========================================================
        if hasattr(self, 'cursor_content'):
            if event.inaxes and event.xdata is not None and event.ydata is not None:
                ...
                # 2. 更新独立的光标坐标显示框 (Max 模式下)
                if param_or_combined == 'COMBINED':
                    self.cursor_content.config(text=f"X: {event.xdata:.3f}, Y: {event.ydata:.3f}")
            else:
                # 1. 更新 Toolbar 消息 (光标不在图内)
                if tb:
                    try:
                        if tb.mode == '':
                            default_msg = "Zoom & Pan enabled (Combined)" if param_or_combined == 'COMBINED' else f"Zoom & Pan enabled for {param_or_combined}"
        ...
        # =========================================================
        # === 原有光标不在图内的 Toolbar 逻辑 (确保兼容) ===
        # =========================================================
        # 这一块主要处理不在图内时的默认消息，上面的逻辑已经覆盖，但为了安全保留检查。
        if not event.inaxes:
    ...
    def enter_max_mode(self):
        ...
        if not getattr(self, "max_frame", None):
            created = True
            print("[DEBUG] Creating max_frame and canvas (fresh)")  # debug
            ...
            try:
                ...
                print("[DEBUG] Failed to create FigureCanvasTkAgg:", e)
                self.max_canvas = None
            ...
            try:
                ...
                print("[DEBUG] toolbar creation failed:", e)
                self.max_toolbar = None

        else:
            # Existing frame: ensure it is visible and that we have axes/canvas
            print("[DEBUG] Re-entering existing max_frame (robust)")  # debug
            try:
                self.max_frame.grid(row=0, column=0, rowspan=2, columnspan=2, sticky="nsew", padx=8, pady=8)
            except Exception:
                pass
            ...
            if getattr(self, "max_fig", None) is None:
                try:
                    ...
                    print("[DEBUG] recreating max_fig failed:", e)
            else:
                if not getattr(self.max_fig, "axes", None):
                    try:
                        self.max_ax = self.max_fig.add_subplot(111)
                    except Exception as e:
                        print("[DEBUG] failed to add subplot to max_fig:", e)
                else:
                    try:
            ...
            if not getattr(self, "max_canvas", None):
                try:
                    print("[DEBUG] max_canvas missing -> recreating FigureCanvasTkAgg")  # debug
                    self.max_canvas = FigureCanvasTkAgg(self.max_fig, master=self.max_frame)
                    self.max_canvas_widget = self.max_canvas.get_tk_widget()
                    self.max_canvas_widget.pack(fill="both", expand=True)
                except Exception as e:
                    print("[DEBUG] recreate max_canvas failed:", e)
                    self.max_canvas = None
                    self.max_canvas_widget = None
        ...
        if canvas_obj:
            try:
                ...
                print("[DEBUG] called max_canvas.draw()")  # debug
            except Exception as e:
                print("[DEBUG] max_canvas.draw() exception:", e)
        ...
        if bind_target is None:
            try:
                print("[DEBUG] No bind_target found: attempting to (re)create FigureCanvasTkAgg")  # debug
                self.max_canvas = FigureCanvasTkAgg(self.max_fig, master=self.max_frame)
                self.max_canvas_widget = self.max_canvas.get_tk_widget()
                self.max_canvas_widget.pack(fill="both", expand=True)
                self.max_canvas.draw()
                bind_target = self.max_canvas
                print("[DEBUG] Recreated max_canvas and drew it; bind_target ready")
            except Exception as e:
                print("[DEBUG] Failed to create bind_target:", e)
                bind_target = None
        ...
        if bind_target is not None:
            try:
                ...
                try:
                    ...
                    print("[DEBUG] bound motion_notify_event to bind_target, cid=", cid)
                except Exception as e:
                    print("[DEBUG] bind_target.mpl_connect failed:", e)
                ...
                print("[DEBUG] Exception during binding to bind_target:", e)
        else:
            print("[DEBUG] WARNING: still no canvas/bind target available after attempts")
        ...
        try:
            ...
            print("[DEBUG] _manage_max_mode_drag_bindings failed:", e)
        ...
        try:
            ...
            print("[DEBUG] draw attempts failed:", e)
        ...
        print("[DEBUG] enter_max_mode end: created=", created, " max_ax:", hasattr(self, 'max_ax') and self.max_ax is not None,
              " max_canvas:", hasattr(self, 'max_canvas') and self.max_canvas is not None,
              " bind_cid:", getattr(self, "_cursor_move_cid", None))

        # Finally, explicitly call update_plots and plot_combined to ensure drawing
        try:
            print("[DEBUG] calling update_plots() from enter_max_mode")  # debug
            self.update_plots()
            # also try calling plot_combined directly if available
            if hasattr(self, "plot_combined"):
                try:
                    print("[DEBUG] calling plot_combined() from enter_max_mode directly")
                    self.plot_combined(redraw_full=True)
                except Exception as e:
                    print("[DEBUG] direct plot_combined call failed:", e)
        except Exception as e:
            print("[DEBUG] update_plots() call failed:", e)
