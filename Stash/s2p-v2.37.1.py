...
class SViewGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("S-View Created By Arthur Gu | V2.36")
    ...
    def add_marker_on_click_normal(self, event, param):
        ...
        最终修复方案：采用最健壮的逻辑，根据插值 Y 轴差值，
        **始终**选择最接近点击位置的 Dataset ID。
        """
        if not self.marker_click_enabled.get(): # <--- 新增检查
            return        
        # 确保点击事件有效且存在数据集
        ...
        # FIXED: 检查是否为左键点击 (button=1)，防止右键时意外添加 Marker
        if event.button != 1:
            return
        ...
        # 2. 寻找最接近点击位置的 Dataset ID (BUG 修复核心)
        # ----------------------------------------------------
        closest_data_id = None
        min_y_diff = float('inf')
        ...
        for dataset in self.datasets:
            ...
            if data_array is not None and len(freq) > 1:
                ...
                if y_interpolated is not None:
                    ...
                    # 寻找最小差值对应的 Dataset ID，并更新
                    # 无需任何容忍度判断，只需找到绝对最小的差异
                    if y_diff < min_y_diff:
                        min_y_diff = y_diff
                        closest_data_id = str(data_id) # 始终更新为最接近的 ID
        ...
        new_mark = {
            ...
            "freq": tk.StringVar(value=f"{f_val:g}"),
            "unit": tk.StringVar(value=f_unit),
            "data_id": tk.StringVar(value=target_data_id) # FIXED: 使用最接近的 data ID
        }
        self.data[plot_type]["marks"][param].append(new_mark)
    ...
    def delete_marker_on_right_click(self, mpl_event, param=None):
        ...
        TOLERANCE_MULTIPLIER = 10.0  # **已修正拼写错误**
        ...
        if self.datasets and len(self.datasets[0]['freq']) > 1:
            ...
            # 注意：这里的变量名已从 TOLERANCE_MULTIPLER 修正为 TOLERANCE_MULTIPLIER
        ...
        # 绝对容忍度：5.0 单位 (用于确保功能恢复。可自行调整 1.0-15.0)
    ...
    def plot_combined(self):
        ...
        for p in self.params:
            ...
            for dataset in self.datasets:
                ...
                #Max模式下曲线的粗细从linewidth=1.5改到linewidth=1.0
                # 获取自定义 ID 名称（若未定义则显示默认 ID）
                custom_name = self.custom_id_names.get(str(data_id), f"ID {data_id}")

                # 绘制曲线，标签显示自定义名称
                ax.plot(freq_mhz, y_data, color=color, linewidth=1.0, label=f"{p} {custom_name}")
        ...
        # 新增：Max模式下应用自定义Y轴限制（使用第一个可见参数的配置作为代表）
        visible_params = [p for p in self.params if self.show_param_vars[p].get()]
        if visible_params and self.y_configs[plot_type][visible_params[0]]["mode"].get() == "Custom":
            try:
        ...
        # 新增：应用自定义 X 轴限制 (Global for combined)
                ...
                for mark in self.data[plot_type]["marks"][p]:
                    try:
                        ...
                        for dataset in self.datasets:
                            ...
                            if val is None:
                                continue
                            x_pt = target_freq_hz / 1e6 # Frequency in MHz
                            y_pt = val
                            ...
                            # Draw marker
                            #Marker标记从'X'改为原点'.'
                            ax.plot(x_pt, y_pt, '.', markerfacecolor='none', markeredgecolor=color, markersize=7, markeredgewidth=1.2, zorder=5)
                            ...
                            marker_legend_text.append(f"{mark['id']} ({p} ID {data_id}) @{x_display}, {val:.3f} {y_unit}")                       
                    except:
                        pass
            ...
            for mark in self.data[plot_type]["marks"][param]:
                try:
                    ...
                    for dataset in self.datasets:
                        ...
                        # Plot marker
                        #Marker标记从'X'改为原点'.'
                        ax.plot(x_pt, y_pt, '.', markerfacecolor='none', markeredgecolor=color, markersize=7, markeredgewidth=1.5, zorder=5)
                        ...
                        marker_legend_text.append(f"{mark_id} (ID {data_id}) @{x_display}, {val:.3f} {y_unit}")
    ...
    def load_s2p(self):
        file_paths = filedialog.askopenfilenames(title="Select S2P Files", filetypes=[("S2P files", "*.s2p"), ("All files", "*.*")])  # 支持多选
        if not file_paths:
            return
        loaded_count = 0
        for file_path in file_paths:
            try:
                ...
                messagebox.showerror("Load Failed", f"Cannot parse S2P file {file_path}:\n{e}")
                continue
        if loaded_count > 0:
            self.status_var.set(f"Loaded {loaded_count} S2P file(s). Total Files: {len(self.datasets)}")
            self.update_file_list_ui()
    ...
    def _handle_chart_output(self, copy=False):
        try:
            ...
            # ------------------- Normal 模式 -------------------
                ...
                title_line1 = f"{self.serial_var.get()}"
    ...
    def set_custom_id_name(self):
        ...
        if not selected_id:
            messagebox.showwarning("Warning", "Please select an ID first.")
            return

        if not new_name:
            messagebox.showwarning("Warning", "Please input a new name.")
            return

        self.custom_id_names[selected_id] = new_name
        self.status_var.set(f"Custom name for ID {selected_id} set to '{new_name}'")
