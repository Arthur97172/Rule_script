...
# 文件: s2p-test1.py (在 SViewGUI 类内，替换 _setup_unified_y_control_ui 方法)
    ...
    def update_plots(self):
        ...
        #self.root.update_idletasks()  # <<<<<  【删除或注释】这一行，将刷新推迟到布局稳定后

        # --- 变量安全初始化（防止 NameError）---
        # 如果 mode != "Normal"，这些变量将保持为空，但不会导致 NameError
        layout_config = {}
        selected_params = [] 
        # ----------------------------------------
        ...
        if not self.datasets:
            # 清空所有 Normal 模式图表
            for p in self.params:
                if p in self.plot_configs: # 检查是否存在以避免 Key Error
                    cfg = self.plot_configs[p]
                    cfg["ax"].clear()
                    # 保持 ax.set_title(p) 避免进一步的 NameError，但它应该被 plot_parameter 覆盖
                    cfg["ax"].set_title(p) 
        ...
        # ----------------------------------------------------
        # 【新增】Normal 模式下的动态布局重建和绘制
        # ----------------------------------------------------
        if mode == "Normal":
            # 1. 清空旧的图表配置和 charts_frame 及其内容
            for widget in self.charts_frame.winfo_children():
                # 检查 Max 模式的 frame 是否存在，如果存在且是当前 widget，则跳过销毁
                if hasattr(self, 'max_frame') and widget is self.max_frame:
                    # 确保它在切换到 Normal 模式时被隐藏
                    if widget.winfo_ismapped(): 
                        widget.grid_forget()
                        widget.pack_forget() 
                    continue
                widget.destroy() # 销毁其他所有 Normal 模式的子组件
                
            self.plot_configs = {} # 清空旧的配置

            # 配置 charts_frame 的 2x2 grid 权重，确保图表可以缩放
            for i in range(2):
            ...
            for param in selected_params:
                ...
                # ------ 创建 Matplotlib 结构（Figure, Axes, Canvas, Frame） ------
                
                # 创建 Frame (用于包裹整个 S 参数图表)
                colors = {"S11": "blue", "S21": "green", "S12": "red", "S22": "purple"}
                frame = tk.LabelFrame(
                    self.charts_frame, 
                    text=" ", # 移除 LabelFrame 标题
                    font=("sans-serif", 11, "bold"), 
                    bg="#f0f2f5", 
                ...
                ax = fig.add_subplot(111)
                # 【优化 1】：增加 padding 以确保轴标签和标题有足够的空间
                # 调整 pad 参数，并使用 auto=True 尝试自动调整
                #fig.tight_layout(pad=0.5) # <--- 原始代码
                ...
                # 引入一个 Frame 容器来放置工具栏
                toolbar_frame = tk.Frame(frame)
                toolbar_frame.pack(side=tk.BOTTOM, fill=tk.X, padx=2, pady=2)
                
                # 添加 Matplotlib 导航工具栏
                toolbar = NavigationToolbar2Tk(canvas, toolbar_frame) # Master 更改为 toolbar_frame
                toolbar.update()
