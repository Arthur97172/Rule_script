...
class SViewGUI:
    ...
    def _on_auto_font_color_change(self, *args):
            """
            处理 auto_font_color_enabled 状态变化时的自动切换逻辑，
            并禁用/启用 Face Color 和 Alpha 的 ComboBox 控件。
            """
            ...
            if is_enabled:
                # 当 auto_font_color_enabled 是 true 时：
                # Face Color设置为none，Alpha设置成0.0
                facecolor_var.set("none")
                alpha_var.set("0.0")
                new_state = 'disabled' # 自动模式开启时禁用手动控件
            else:
                # 当 auto_font_color_enabled 是 false 时：
                # Face Color设置为Default，Alpha设置成Default
                facecolor_var.set("Default")
                alpha_var.set("Default")
                new_state = 'readonly' # 自动模式关闭时启用手动控件
                
            # 禁用/启用 Face Color 和 Alpha 的 ComboBox 控件
            # 前提是这些控件的引用已在 UI setup 函数中保存到 self.marker_legend_ui_refs
            if 'facecolor_combo' in self.marker_legend_ui_refs:
                self.marker_legend_ui_refs['facecolor_combo']['state'] = new_state
            if 'alpha_combo' in self.marker_legend_ui_refs:
                self.marker_legend_ui_refs['alpha_combo']['state'] = new_state
...
# 文件: s2p-test1.py (在 SViewGUI 类内，替换 _setup_unified_y_control_ui 方法)
    ...
    def load_s2p(self):
        ...
        def custom_load_dialog():           
            ...
            dialog.title("Select Loading Method")
    ...
    def _create_marker_legend_config_ui(self, master_frame):
        ...
        self.marker_legend_ui_refs['facecolor_combo'] = facecolor_combo # <-- 【新增此行】
        ...
        self.marker_legend_ui_refs['alpha_combo'] = alpha_combo # <-- 【新增此行】
