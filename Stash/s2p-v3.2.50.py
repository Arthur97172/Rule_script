...
# 颜色循环
#1f77b4 (#经典蓝), #ff7f0e (#经典橙), #2ca02c (#经典绿), #d62728 (#经典红), #9467bd (#经典紫), #8c564b (#棕褐色), #e377c2 (#玫瑰粉), #bcbd22 (#黄绿色), #17becf (#青蓝色), #7f7f7f (#中灰色),#e41a1c (#哈佛红),#377eb8 (#亮宝蓝), #4daf4a (#鲜草绿), #984ea3 (#明亮紫), #ff7f00 (#亮橙色), #ffff33 (#明黄色), #a65628 (#深棕色), #f781bf (#樱花粉), #999999 (#浅灰色), #66c2a5 (#薄荷绿)
...
class SViewGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("S-View Created By Arthur Gu | V3.2")
        ...
        # 1. 检查文件是否存在，防止打包后找不到文件
        if not os.path.exists(icon_path):
            # 兼容 pyinstaller 打包后的路径（它会将文件放入临时目录）
            # pyinstaller 在运行时会设置一个特殊的 _MEIPASS 属性
            if hasattr(sys, '_MEIPASS'):
                 icon_path = os.path.join(sys._MEIPASS, "StoreLogo.ico")
            
        try:
            # 使用 iconbitmap() 方法加载 .ico 文件
            self.root.iconbitmap(icon_path)
            
        except tk.TclError:
            # 如果加载失败（例如路径不正确或文件损坏），这里可以捕获错误
...
# 文件: s2p-test1.py (在 SViewGUI 类内，替换 _setup_unified_y_control_ui 方法)
    ...
    def plot_combined(self, redraw_full=True):
                ...
                for line in self.data[plot_type]["limit_lines"].get(p, []):
                    try:
                        ...
                        if min_f < max_f:
                            ...
                            # Y 坐标轴上的起点和终点 (线性插值)
                            # 如果 start_mhz == stop_mhz，则为水平线，y_line_start = y_line_stop = (lower + upper) / 2
                            if abs(start_mhz - stop_mhz) < 1e-6: # 检查是否为水平线（频率相同）
                                y_line_start = lower
                                y_line_stop = upper # 实际上此时 lower 和 upper 相同没关系，但为了逻辑统一，我们不插值
                                # 考虑到您的需求是斜线，如果频率相同，则它们应该是水平的，但用户输入的 lower/upper 仍可能不同。
                                # 由于 UI 设计是 (Start Freq, Lower) 到 (Stop Freq, Upper)，如果 Start=Stop，则它应该只是一条垂直的线，但我们只画水平/斜线。
                                # 假设 Start != Stop
        ...
        if visible_marker_info_list:
            ...
            PARAM_ORDER = {'S11': 0, 'S21': 1, 'S12': 2, 'S22': 3}
            def sort_key(info):
                mark_id, param, _, data_id_str = info
                try: data_id = int(data_id_str)
                except: data_id = 999999
                param_idx = PARAM_ORDER.get(param, 99)
                marker_num = getattr(self, 'get_marker_id_number', lambda x: 0)(mark_id)
    ...
    # 加载s2p文件，支持单文件/多文件/文件夹批量加载
    def load_s2p(self):
        # --- 自定义对话框函数 ---
        def custom_load_dialog():           
            # 1. 创建顶层窗口
            ...
            # 定义按钮样式，ttk默认主题在许多OS上是圆角的。
            style = ttk.Style(dialog)
            # 使用 clam 主题，通常比 default 主题更现代
            style.theme_use('clam') 
            
            # 可以尝试设置按钮的边框半径，但这在所有系统/主题上不一定生效
            ...
            # 2. 使其成为模态对话框，并暂时禁用主窗口
            dialog.grab_set() 
            
            # 3. 结果变量，用于存储用户选择 (None, 'files', 'folder')
            result = None
            ...
            # 5. 界面元素 - 【调整 pady 增加顶部间距】
            label = tk.Label(dialog, text="How would you like to load the S2P files?", 
                             pady=15) # 调整 pady 从 10 到 15
            label.pack()

            # 6. 按钮框架 - 【调整 pady 增加上下间距】
            button_frame = tk.Frame(dialog)
            button_frame.pack(pady=20) # 调整 pady 从 10 到 20 (向下推动按钮)

            # Files 按钮 (使用 ttk.Button 和自定义样式)
            btn_files = ttk.Button(button_frame, text="Files", width=8,
                                   style='Rounded.TButton', # 应用自定义样式
                                   command=lambda: on_button_click('files'))
            btn_files.pack(side=tk.LEFT, padx=10)
            
            # Folder 按钮 (使用 ttk.Button 和自定义样式)
            btn_folder = ttk.Button(button_frame, text="Folder", width=8,
                                    style='Rounded.TButton', # 应用自定义样式
                                    command=lambda: on_button_click('folder'))
            btn_folder.pack(side=tk.LEFT, padx=10)

            # Cancel 按钮 (使用 ttk.Button 和自定义样式)
            btn_cancel = ttk.Button(button_frame, text="Cancel", width=8,
                                    style='Rounded.TButton', # 应用自定义样式
        ...
        # ------------------------------------------------------------
        # 1. Ask User for Loading Method (Custom Dialog)
        # ------------------------------------------------------------
        
        choice = custom_load_dialog() # choice 将是 'files', 'folder', 或 None
    ...
    def open_color_setting_dialog(self, dataset_id):
        """颜色设置对话框：Normal 模式立即生效，Max 模式需要确认"""
        if not hasattr(self, 'custom_id_colors'):
            self.custom_id_colors = {}
        if not hasattr(self, 'custom_sparam_colors'):
            self.custom_sparam_colors = {}
        ...
        if mode == "Normal":
            ...
            tk.Label(win, text=f"Set color for ID {dataset_id}:", font=("sans-serif", 11, "bold")).pack(pady=15)

            # 当前颜色
            current_color = self.custom_id_colors.get(dataset_id, COLOR_CYCLE[(dataset_id - 1) % len(COLOR_CYCLE)])

            # 实时预览框
            preview = tk.Label(win, bg=current_color, width=6, height=1, relief="sunken", bd=2)
            ...
            tk.Label(win, text=f"Set S-Parameter colors for ID {dataset_id}:", 
                    font=("sans-serif", 11, "bold")).pack(pady=15)
            ...
            for param in params:
                # 获取当前颜色（自定义 > 默认循环色）
                default_idx = self.params.index(param)
                current = self.custom_sparam_colors[dataset_id].get(param, COLOR_CYCLE[default_idx % len(COLOR_CYCLE)])
                temp_colors[param] = current

                row = tk.Frame(win)
                row.pack(fill="x", pady=7, padx=20)

                tk.Label(row, text=param, width=6, font=("sans-serif", 10, "bold")).pack(side="left")
                lbl = tk.Label(row, bg=current, width=6, height=1, relief="groove", bd=2)
            ...
            tk.Button(btn_frame, text="Confirm", width=12, height=2, bg="#27ae60", fg="white", 
                     font=("sans-serif", 10), command=apply).pack(side="left", padx=15)
            tk.Button(btn_frame, text="Cancel", width=11, height=2, bg="#e74c3c", fg="white",
