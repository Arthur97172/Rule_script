...
#1.1 Fixed bug
#2.0 Added Max Mode
#2.1 Added Limit line support
#2.2 Added Marker support
...
#2.47.23 移动Marker Legend Position位置到File & Settings标签页
...
#4.0 修复bug,对S3P,S4P的支持
#4.0.1 修复Reset App系统奔溃bug
#修复port pair不为1-2时无法编辑limits & Marks参数的bug
#修改关键按钮和标签改名及支持PDF格式
#加载一个文件是Normal模式截图、保持图片不显示ID信息
#修复隐藏文件后从Normal切换到Max不隐藏的bug
#修复port pair为1-2时修改Legend Position自动跳转到S11的bug
#修复Hide文件后再次加载相同文件始终被Hide的bug并加入自动回到port pair为1-2
#4.0.2 修复Max模式下Hide文件后拖拽Marker被Hide的文件又被显示的bug
#修复Normal模式下添加Marker后切换到Max模式无法拖动的bug
#4.0.3 修复Max模式下复位没有恢复M和S Legend position的bug
#4.0.4 添加Limit Line自定义颜色
#4.0.5 优化按钮颜色和按钮大小
#4.0.6 添加隐藏Hide Y-Axis Title功能
#4.0.7 优化Normal和Max模式的Y轴刻度
#4.1.0 增加Wrapped Phase功能
#4.1.1 修复Max模式添加logo保存文件重叠的问题
#4.1.2 修复选择文件后ID编号错误的问题
...
class ActivationDialog(simpledialog.Dialog):
    ...
    def body(self, master):
        ...
        self.after(100, lambda: set_app_icon(self))
...
class SViewGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("S-View Created By Arthur Gu | V4.1.2")
        ...
        plt.rcParams['figure.dpi'] = self.actual_dpi
        plt.rcParams['savefig.dpi'] = 300

        # ======================== 【关键新增：Blitting 拖拽专用状态变量】 ========================
        # 用于 Marker Legend 拖动时的超高速 Blitting 渲染
        self.drag_legend_bg = {}            # {axes: background} 缓存每个子图的背景
        self.drag_legend_ax = None          # 当前正在拖动的 axes
        self.drag_legend_artist = None      # 当前正在拖动的 Artist（Text 或 AnnotationBbox）
        self._drag_blit_cache_valid = False # 背景缓存是否有效（每次按下时强制刷新一次最稳）
        ...
        self.report_logo_width = tk.StringVar(value="120")
        self.report_logo_height = tk.StringVar(value="48")
        self.max_fig = None
        ...
        self.marker_click_enabled = tk.BooleanVar(value=False)
        self.freeze_refresh_var = tk.BooleanVar(value=False)
        self.limits_check_enabled = tk.BooleanVar(value=False)
        self.auto_font_color_enabled = tk.BooleanVar(value=False)
        self.auto_font_color_enabled.trace_add("write", self._on_auto_font_color_change)

        # [新增] Y 轴标签隐藏控制
        self.hide_ytitle_var = tk.BooleanVar(value=False)
        self.hide_ytitle_var.trace_add("write", lambda *args: self.replot_after_config_change())

        #Port Pair端口监听
        self._port_pair_change_trace_id = self.port_selection_var.trace_add("write", self.on_port_pair_change)

        # 【新增】: 为 marker_click_enabled 添加 trace，监听变化并动态更新 Max 拖动事件
        self.marker_click_enabled.trace_add("write", self._on_marker_click_enabled_change)
        ...
        self.DRAG_UPDATE_INTERVAL = 30  # Blitting 下可以更激进，30ms ≈ 33FPS 都丝滑

        # Logo 相关变量
        self.report_logo_x_offset = tk.StringVar(value="10") 
        self.report_logo_y_offset = tk.StringVar(value="10")  

        # ======================== 初始化核心状态 ========================
        self._initialize_state()

        # 其余你原来的代码全部保留（custom_id_names、cid_max_drag_xxx 等）
    ...
    def _initialize_state(self):
        ...
        SUPPORTED_PLOT_TYPES = ["Magnitude (dB)", "Phase (deg)", "Wrapped Phase (deg)", "Group Delay (ns)"]
        ...
        self.drag_param = None  # Normal 模式下记录是哪个 param 的 Legend
        ...
        y_defaults = {
            ...
            "Wrapped Phase (deg)": {"min": "-180", "max": "180"},            
        ...
        for pt in ["Magnitude (dB)", "Phase (deg)", "Wrapped Phase (deg)", "Group Delay (ns)"]:
            for p in self.params:
        ...
        # 最后加上这几句，确保每次切换 plot_type 时也重置 Blitting 缓存（推荐）
        def _reset_blit_cache(*args):
            self._drag_blit_cache_valid = False
            self.drag_legend_bg.clear()
        self.plot_type.trace_add("write", _reset_blit_cache)
        self.display_mode.trace_add("write", _reset_blit_cache)
    # ----------------------------------------------------

    def get_visible_datasets(self):
        return [ds for ds in self.datasets if ds.get('is_displayed', True)]
    ...
    def _calculate_phase_deg(self, s_data, wrapped=False):
        """
        Calculate phase in degrees.

        wrapped = False -> Unwrapped Phase (original behavior)
        wrapped = True  -> Wrapped Phase (-180 ~ 180)
        """
        phase = np.angle(s_data) * 180 / np.pi
        if not wrapped:
            # 原有 Phase (deg) 行为：Unwrapped
            phase = np.unwrap(phase * np.pi / 180) * 180 / np.pi
        return phase

    def _is_phase_plot(self, plot_type):
        return plot_type in ("Phase (deg)", "Wrapped Phase (deg)")

    def _is_wrapped_phase(self, plot_type):
        return plot_type == "Wrapped Phase (deg)"


    # --- 新增 Limit Line 颜色管理方法 ---
    def get_limit_line_color(self, plot_type, param):
        """获取指定参数的限制线颜色，如果未设置则返回默认蓝色。"""
        # self.data["Magnitude (dB)"]["limit_line_colors"] = {"S11": "#ff0000", ...}
        if plot_type in self.data and "limit_line_colors" in self.data[plot_type]:
            # 使用 .get() 尝试获取自定义颜色，否则返回默认 'blue'
            return self.data[plot_type]["limit_line_colors"].get(param, 'blue')
        return 'blue' # 全局默认颜色

    def _init_limit_line_color_storage(self, plot_type):
        """确保 limit_line_colors 字典已初始化。"""
        if plot_type not in self.data:
            self.data[plot_type] = {}
        if "limit_line_colors" not in self.data[plot_type]:
            self.data[plot_type]["limit_line_colors"] = {}
            
    def _on_custom_limit_color(self, plot_type, param):
        """弹出颜色选择器，自定义指定 S 参数下所有 Limit Line 的颜色。"""
        initial_color = self.get_limit_line_color(plot_type, param)
        
        color_code = colorchooser.askcolor(color=initial_color, 
                                           title=f"Select Custom Color for {param} Limit Lines in {plot_type}")
        
        if color_code and color_code[1]:
            new_color = color_code[1]
            self._init_limit_line_color_storage(plot_type)
            
            # 存储新颜色
            self.data[plot_type]["limit_line_colors"][param] = new_color
            
            # ✅ 第 1 步：更新 UI 颜色显示框 (立即刷新显示框，满足用户要求)
            if "limit_color_display" in self.data[plot_type]["ui_refs"][param]:
                self.data[plot_type]["ui_refs"][param]["limit_color_display"].config(bg=new_color)

            # ⚠️ 第 2 步：刷新图表
            # 必须保留 self.update_plots()，否则图表上的 Limit Line 颜色不会改变。
            #self.update_plots() 
            
            if hasattr(self, 'status_var'):
                self.status_var.set(f"Limit Line color for {param} set to {new_color}")

    def _on_reset_limit_color(self, plot_type, param):
        """将指定 S 参数下所有 Limit Line 的颜色恢复为默认蓝色。"""
        default_color = 'blue'
        
        self._init_limit_line_color_storage(plot_type)
        
        # 从字典中删除条目，让 get_limit_line_color 返回默认值
        if param in self.data[plot_type]["limit_line_colors"]:
            del self.data[plot_type]["limit_line_colors"][param]
            
        # ✅ 第 1 步：更新 UI 颜色显示框 (立即刷新显示框，满足用户要求)
        if "limit_color_display" in self.data[plot_type]["ui_refs"][param]:
            self.data[plot_type]["ui_refs"][param]["limit_color_display"].config(bg=default_color)
        ...
        if hasattr(self, 'status_var'):
            self.status_var.set(f"Limit Line color for {param} restored to default '{default_color}'")

    def update_limit_color_button_visibility(self, plot_type, param):
        """根据是否存在 Limit Line 来控制颜色按钮的显示/隐藏。"""
        # 1. 检查 UI 引用是否存在
        if (param not in self.data[plot_type]["ui_refs"] or 
            "limit_color_container" not in self.data[plot_type]["ui_refs"][param]):
            return
            
        container = self.data[plot_type]["ui_refs"][param]["limit_color_container"]
        
        # 2. 检查 Limit Line 数量
        limit_lines = self.data[plot_type]["limit_lines"].get(param, [])
        
        if limit_lines:
            # 如果有 Limit Line，显示按钮 (靠左侧打包)
            container.pack(side="left", padx=10)
        else:
            # 如果没有 Limit Line，隐藏按钮
            container.pack_forget()
            # 同时清除自定义颜色（恢复默认，可选但推荐）
            self._init_limit_line_color_storage(plot_type)
            if param in self.data[plot_type]["limit_line_colors"]:
                del self.data[plot_type]["limit_line_colors"][param]
    ...
    #Port Pair监听
    def on_port_pair_change(self, *args):
        """
        Port Pair 变化时，更新 Limits & Marks、Marker Position、Show Params 和图表。
        """
        # 更新 Limits & Marks 的 S 参数（假设代码中有此函数，根据附件有调用痕迹）
        if hasattr(self, 'update_limit_mark_params'):
            self.update_limit_mark_params()
        
        # 更新 Marker Position UI（已存在）
        if hasattr(self, 'update_marker_position_ui'):
            self.update_marker_position_ui()
        
        # 重建 Show Params UI（已存在，防止端口变化后复选框不匹配）
        ...
        # 刷新 Limits & Marks 标签页显示（如果当前是隐藏的plot_type，确保可见）
        self.on_plot_type_change()  # 借用现有逻辑，确保tab状态更新
        
        # 强制刷新 Tkinter 渲染队列，防止焦点问题
    ...
    def _check_dataset_limits(self, dataset, plot_type, param):
        ...
        elif self._is_phase_plot(plot_type):
            y_data = self._calculate_phase_deg(
                s_data,
                wrapped=self._is_wrapped_phase(plot_type)
            )
            freq_mhz = freq_hz / 1e6
    ...
    def _update_max_marker_display(self, redraw_points=True):
        ...
        # ----------------------------------------------------
        if hasattr(self, 'max_marker_legend_artists') and plot_type in self.max_marker_legend_artists:
            if self.max_marker_legend_artists[plot_type]:
                try:
        ...
        # ----------------------------------------------------
        # 2. 清除旧的 Marker 点和 Annotation Artists
        # ----------------------------------------------------
        ...
        # ----------------------------------------------------
        # 3. 获取当前 X 轴可见范围（用于判断 Marker 是否在视图内）
        # ----------------------------------------------------
        try:
        ...
        # ----------------------------------------------------
        # 4. 核心：只处理【可见文件】的 Marker
        # ----------------------------------------------------
        if plot_type not in self.data:
            return

        # 提前构建一个 data_id → dataset 的快速映射（避免每次 next() 遍历）
        dataset_map = {str(ds['id']): ds for ds in self.datasets}
        ...
        for p in visible_params:
            for mark in self.data[plot_type]["marks"].get(p, []):
                try:
                    ...
                    dataset = dataset_map.get(selected_data_id)
                    if not dataset:
                        continue

                    # 【关键修复】：如果该文件被隐藏，直接跳过！
                    if not dataset.get('is_displayed', True):
                        continue
                    ...
                    if plot_type == "Magnitude (dB)":
                        ...
                        data_array = self._calculate_phase_deg(
                            s_data,
                            wrapped=self._is_wrapped_phase(plot_type)
                        )
                    elif plot_type == "Group Delay (ns)":
                    ...
                    # 插值
                    min_f_hz, max_f_hz = freq[0], freq[-1]
                    in_data_range = min_f_hz <= target_freq_hz <= max_f_hz
                    val = self.safe_interp(target_freq_hz, freq, data_array) if in_data_range else None

                    # 准备 Legend 文本
                    color = self.get_max_mode_color(dataset['id'], p)
                    custom_name = self.custom_id_names.get(selected_data_id, f"ID {selected_data_id}")
                    y_str = f"{val:.3f} {y_unit}" if val is not None and in_data_range else "Out of Freq Range"
                    full_legend_text = f"{mark_id} ({p} {custom_name}) @{x_display}, {y_str}"
                    marker_info_list.append((mark_id, p, full_legend_text, selected_data_id))

                    # 只在可见范围内绘制 Marker 点
                    if not in_data_range or val is None:
                        continue

                    x_pt_mhz = target_freq_hz / 1e6
                    if not (x_min_mhz <= x_pt_mhz <= x_max_mhz):
                        continue

                    # 绘制 Marker 点和标签
                    line = ax.plot(x_pt_mhz, val, '.', markerfacecolor='none',
                                  markeredgecolor=color, markersize=4, markeredgewidth=2, zorder=5)
                    text = ax.annotate(mark_id, xy=(x_pt_mhz, val), xytext=(5, 5),
                    ...
                    # 单个 Marker 出错不影响整体
                    print(f"[Marker Draw Error] {e}")
                    continue

        # ----------------------------------------------------
        # 5. 绘制 Marker Legend（黄色信息框）
        # ----------------------------------------------------
        if marker_info_list:
            # 排序：先按 ID，再按 S11 > S21 > S12 > S22
            def sort_key(item):
                mark_id, param, _, data_id_str = item
                try:
                    data_id_num = int(data_id_str)
                except:
                    data_id_num = 9999
                param_order = {'S11': 0, 'S21': 1, 'S12': 2, 'S22': 3}.get(param, 99)
                return (data_id_num, param_order)

            sorted_info = sorted(marker_info_list, key=sort_key)
            legend_lines = [info[2] for info in sorted_info]
            txt = "\n".join(legend_lines)

            # 位置配置
            cfg = self.max_marker_pos_configs[plot_type]
            mode = cfg["mode_var"].get()
            x_val, y_val = 0.98, 0.98
            ha, va = 'right', 'top'

            if mode == "Top Left":
                x_val, y_val, ha, va = 0.02, 0.98, 'left', 'top'
            elif mode == "Top Right":
                x_val, y_val, ha, va = 0.98, 0.98, 'right', 'top'
            elif mode == "Bottom Left":
                x_val, y_val, ha, va = 0.02, 0.02, 'left', 'bottom'
            elif mode == "Bottom Right":
                x_val, y_val, ha, va = 0.98, 0.02, 'right', 'bottom'
            elif mode == "Center":
                x_val, y_val, ha, va = 0.5, 0.5, 'center', 'center'
            elif mode == "Custom":
                try:
                    x_val = float(cfg["x_var"].get())
                    y_val = float(cfg["y_var"].get())
                    ha = 'left' if x_val < 0.5 else 'right'
                    va = 'bottom' if y_val < 0.5 else 'top'
                    if abs(x_val - 0.5) < 0.01: ha = 'center'
                    if abs(y_val - 0.5) < 0.01: va = 'center'
                except:
                    pass

            bbox_params = self._get_marker_legend_bbox_params()

            txt_artist = ax.text(x_val, y_val, txt,
                                 transform=ax.transAxes,
                                 fontsize=9,
                                 va=va, ha=ha,
                                 multialignment='left',
        ...
        # 6. 刷新画布
        # ----------------------------------------------------
        self.max_canvas.draw_idle()  # 推荐使用 draw_idle()，更平滑
    #------------------------------------------   
        ...
        for pt in SUPPORTED_PLOT_TYPES:
            ...
            for p in self.params:
                ...
                tk.Button(custom_y_frame, text="Apply",
                          command=self.update_plots, bg="#3498db", fg="white", width=10).pack(side="left", padx=10)
    ...
    #Y轴标签隐藏刷新辅助
    def replot_after_config_change(self):
        """
        这个方法被 Tkinter 变量的 trace 机制调用，用于在配置（如 hide_ytitle_var）改变时触发图表重绘。
        它应该调用您 SViewGUI 类中实际用于全局更新所有图表的函数。
        """
        # 常见的名称可能是 self.update_plots() 或 self.replot()
        if hasattr(self, 'update_plots'):
            self.update_plots()
        else:
            # 如果没有全局更新函数，至少尝试刷新画布，但强烈建议定义一个全局更新函数。
    ...
    def _refresh_y_axis_tabs(self):
        ...
        for pt in SUPPORTED_PLOT_TYPES:
            ...
            for p in params_to_use:
                ...
                tk.Button(custom_y_frame, text="Apply", bg="#3498db", fg="white", command=self.update_plots,
    ...
    def _ensure_plot_configs_exist(self, new_params):
        ...
        """
        plot_types = ["Magnitude (dB)", "Phase (deg)", "Wrapped Phase (deg)", "Group Delay (ns)"]
        ...
        for param in new_params:
            ...
            for plot_type in plot_types:
                ...
                if plot_type == "Phase (deg)":
                    ...
                    default_limits = {"min": "-180", "max": "180"}
                elif plot_type == "Wrapped Phase (deg)":
                    # Wrapped Phase 模式默认范围：-180 到 180
                    default_limits = {"min": "-180", "max": "180"} 
                elif plot_type == "Group Delay (ns)":
    ...
    #Feature Controls组伸缩功能
    def add_collapsible_section(self, parent_frame, title):
        """
        创建一个可折叠/展开的 ttk.Frame 部分。
        返回：内容框架 (content_frame), 切换函数 (toggle_section), 展开状态变量 (is_expanded)
        """
        # 1. 标题/头部框架 (Header Frame)
        header_frame = ttk.Frame(parent_frame, relief="groove")
        header_frame.pack(fill='x', padx=5, pady=(5, 0))

        # 2. 内容框架 (Content Frame)
        content_frame = ttk.Frame(parent_frame)
        content_frame.pack(fill='both', padx=5, pady=(0, 5))
        
        # 3. 切换状态变量和指示符
        is_expanded = tk.BooleanVar(value=True) # <<< 返回此变量
        indicator_label = ttk.Label(header_frame, text="▼", width=2, anchor='center')
        indicator_label.pack(side="left", padx=(5, 0))
        
        title_label = ttk.Label(header_frame, text=title, font=('sans-serif', 10, 'bold'))
        title_label.pack(side="left", fill="x", expand=True, padx=(0, 5))
        
        # 4. 切换逻辑
        def toggle_section(event=None): # <<< 返回此函数
            if is_expanded.get():
                content_frame.pack_forget()
                indicator_label.config(text="▶")
                is_expanded.set(False)
            else:
                # 关键修复：使用 after=header_frame 保持 pack 顺序
                content_frame.pack(fill='both', padx=5, pady=(0, 5), after=header_frame)
                indicator_label.config(text="▼")
                is_expanded.set(True)

        # 5. 绑定点击事件
        header_frame.bind("<Button-1>", toggle_section)
        indicator_label.bind("<Button-1>", toggle_section)
        title_label.bind("<Button-1>", toggle_section)
        
        # 6. 鼠标悬停时的样式 (移除手性保持不变)
        #title_label.bind("<Enter>", lambda e: title_label.config(cursor="hand2"))
        #title_label.bind("<Leave>", lambda e: title_label.config(cursor=""))

        # 7. 返回内容框架、切换函数和状态变量
        return content_frame, toggle_section, is_expanded
    ...
    def setup_ui(self):
        ...
        plot_values = ["Magnitude (dB)", "Phase (deg)", "Wrapped Phase (deg)", "Group Delay (ns)"]
        ...
        #              bg="#FF9800", fg="white", relief="raised", anchor="w",
        #              padx=left_margin, pady=6, command=refresh_plots_and_reset_flag)\
        #        .pack(fill="x", padx=5, pady=2)
        #居中显示    
        tk.Button(refresh_ops_group, text="Refresh Plots", font=("sans-serif", 10, "bold"),
                          bg="#3498db", fg="white", relief="raised", 
                          anchor="center", # 关键修改 1: 文本居中
                          padx=left_margin, pady=6, 
                          command=refresh_plots_and_reset_flag)\
                    .pack(fill="x", padx=(5, 5), pady=2) # 关键修改 2: 移除 fill="x"

        # 1. 使用 add_collapsible_section 创建可折叠区域
        # 捕获返回的控制变量和函数
        feature_content_container, self.feature_controls_toggle, self.feature_controls_expanded = \
            self.add_collapsible_section(control_stack_frame, "Feature Controls")
        
        # 2. 在内容主框架内创建原有的内部框架
        # 用于容纳所有Checkbutton并设置背景色。
        # 后续的控件将继续使用 inner_combined_frame 作为父级，结构保持一致。
        inner_combined_frame = tk.Frame(feature_content_container, bg="#f0f2f5")
        inner_combined_frame.pack(anchor='w', fill='x', padx=5, pady=2)

        # 1. Add Tag Control: Marker Edit Mode
        tk.Checkbutton(inner_combined_frame,
                                  text="Marker Edit Mode",
                                  variable=self.marker_click_enabled,
                                  bg="#f0f2f5",
                                  anchor='w', 
                                  justify='left').pack(anchor='w', padx=(5, 0), pady=0)  

        # 2. Auto Refresh: Freeze Refresh
        tk.Checkbutton(inner_combined_frame,
                                  text="Freeze Refresh",
                                  variable=self.freeze_refresh_var,
                                  bg="#f0f2f5",
                                  anchor='w', 
                                  justify='left').pack(anchor='w', padx=(5, 0), pady=0)

        # 3. Limits Check: Auto Limits Check
        tk.Checkbutton(inner_combined_frame,
                                  # 为了清晰，将 "Enable Limits Check" 改为 "Auto Limits Check"
                                  text="Auto Limits Check", 
        ...
        # 4. Auto Font Color: Marker Color Sync
        tk.Checkbutton(inner_combined_frame,
                                  text="Marker Color Sync",
        ...
        # 5. Hide Y-Axis Title (More X-Axis Space)
        tk.Checkbutton(inner_combined_frame,
                                  text="Hide Y-Axis Title",
                                  variable=self.hide_ytitle_var,
                                  bg="#f0f2f5",
                                  anchor='w', 
                                  justify='left').pack(anchor='w', padx=(5, 0), pady=0)
        ...
        # Copy Plot 按钮
        tk.Button(copy_save_frame, text="Copy Plot", font=("sans-serif", 9, "bold"), bg="#3F51B5", fg="white", relief="raised", padx=1, pady=6, command=self.copy_all_charts)\
        .pack(side="left", fill="x", expand=True, padx=(0, 2), pady=0)
        # Save Image 按钮
        tk.Button(copy_save_frame, text="Save Plot", font=("sans-serif", 9, "bold"), bg="#9C27B0", fg="white", relief="raised", padx=1, pady=6, command=self.save_chart)\
        ...
        # Load Files 按钮 (放在 load_reset_frame 的左侧)
        tk.Button(load_reset_frame, text="Load Files", font=("sans-serif", 9, "bold"), 
                  bg="#2ecc71", fg="white", relief="raised", padx=1, pady=6, command=self.load_snp)\
        .pack(side="left", fill="x", expand=True, padx=(0, 2), pady=0)

        # Reset App 按钮 (保持在 load_reset_frame 的右侧)"#3F51B5",bg="#e74c3c",bg="#FF5722"
        tk.Button(load_reset_frame, text="Reset App", font=("sans-serif", 9, "bold"), bg="#e74c3c", fg="white", relief="raised", padx=1, pady=6, command=self.reset_application)\
        .pack(side="left", fill="x", expand=True, padx=(2, 0), pady=0)

        # --- Legend frame ---
        self.legend_frame = tk.LabelFrame(vertical_control_frame, text="Data Summary",
                                             font=("sans-serif", 10, "bold"), bg="#f0f2f5")
    ...
    def create_axis_control_tab(self):
        ...
        tk.Button(custom_x_frame, text="Apply", bg="#3498db", fg="white",
    ...
    def _setup_unified_y_control_ui(self, parent_frame):
        ...
        # 刷新按钮 (Apply button)
        tk.Button(custom_y_frame, text="Apply", bg="#3498db", fg="white",
                  command=self.update_plots, width=10).pack(side="left", padx=(20, 5))
    ...
    # =========================================================
    # 【更新】: 监听 marker_click_enabled 变化，只重置 Blitting（拖动事件总是连接，不干预）
    # =========================================================
    def _on_marker_click_enabled_change(self, *args):
        if self.display_mode.get() == "Max":
            # 只在 Max 模式下响应，重置 Blitting 缓存（确保下次拖动丝滑）
            # 无需连接/断开事件：让事件总是连接，press 内部控制
            self._drag_blit_cache_valid = False
            self.drag_legend_bg.clear()

    # 【新增】: 断开 Max 拖动事件（独立函数，用于禁用时调用）
    # =========================================================
    def _disconnect_max_drag_events(self):
        if self.max_canvas:
            for cid_name in ('cid_max_drag_press', 'cid_max_drag_motion', 'cid_max_drag_release'):
                cid = getattr(self, cid_name, None)
                if cid:
                    self.max_canvas.mpl_disconnect(cid)
                    setattr(self, cid_name, None)

        # 重置拖动状态
        self.dragging_marker_legend = False
        self.drag_legend_ax = None
        self.drag_legend_artist = None

        # 如果 marker_click_enabled 为 False，重置 Blitting 缓存（防止禁用后残留拖动）
        if not self.marker_click_enabled.get():
            self._drag_blit_cache_valid = False
            self.drag_legend_bg.clear()

    # =========================================================
    # 【更新】: 统一断开所有拖动事件（增强版，添加 marker_click_enabled 检查）
    # =========================================================
    def _disconnect_all_drag_events(self):
        # 断开 Max 事件
        self._disconnect_max_drag_events()

        # 断开 Normal 事件的逻辑（如果有的话，视您实现而定）
        # 示例（根据您实际的 Normal 拖动实现补充）：
        # for p in self.params:
        #     if p in self.plot_configs and self.plot_configs[p]["canvas"]:
        #         canvas = self.plot_configs[p]["canvas"]
        #         for cid in self.plot_configs[p].get("drag_cids", []):
        #             canvas.mpl_disconnect(cid)

        # 重置拖动状态（已在 _disconnect_max_drag_events 中处理）


    # =========================================================
    # 【更新】: 连接 Max 拖动事件（总是连接，press 内部的 enabled 检查控制行为）
    # =========================================================
    def _connect_max_drag_events(self):
        if not self.max_canvas:
            return  # Canvas 未就绪

        # 先断开旧的（防重复）
        self._disconnect_max_drag_events()

        # 总是重新连接（不受 enabled 影响）
        self.cid_max_drag_press = self.max_canvas.mpl_connect(
            'button_press_event', self.on_marker_legend_press)
        self.cid_max_drag_motion = self.max_canvas.mpl_connect(
            'motion_notify_event', self.on_marker_legend_motion)
        self.cid_max_drag_release = self.max_canvas.mpl_connect(
            'button_release_event', self.on_marker_legend_release)

        # 强制刷新绘图，确保 Artist pickable 更新
        if self.max_fig:
 ...
 # Max mode添加Marker信息
    def add_marker_on_click_combined(self, mpl_event):
        ...
        for param in self.params:
            ...
            for dataset in self.get_visible_datasets():
                data_id = dataset['id']
                freq = dataset['freq']
        ...
        # -------------------------- 刷新逻辑修复 --------------------------
        # 检查 'Auto Refresh' 状态。
        if not self.freeze_refresh_var.get():
    ...
    def on_marker_legend_motion(self, mpl_event):
        ...
        # 关键：只在第一次移动时保存背景（Blitting 初始化）
        if not self._drag_blit_cache_valid or self.drag_legend_ax != ax:
            self.drag_legend_ax = ax
            self.drag_legend_artist = artist
            self.drag_legend_bg[ax] = ax.figure.canvas.copy_from_bbox(ax.bbox)
            self._drag_blit_cache_valid = True

        point = ax.transAxes.inverted().transform((mpl_event.x, mpl_event.y))
        ...
        # 1. 恢复背景（擦除旧位置）
        canvas.restore_region(self.drag_legend_bg[ax])
        # 2. 重绘当前 Artist（只画 Legend）
        ax.draw_artist(artist)
        # 3. 只更新 Artist 所在的矩形区域（超快！）
        canvas.blit(ax.bbox)
        # ============ Blitting 结束 ============

        # 节流：每 16ms 更新一次（约60FPS），防止事件堆积
        if self._drag_update_id:
            self.root.after_cancel(self._drag_update_id)
        self._drag_update_id = self.root.after(16, lambda: None)  # 占位，实际不执行
    #---------------------------------------
    ...
    def _throttled_update_plots(self):
        ...
        if self.freeze_refresh_var.get():
    ...
    def on_marker_legend_release(self, mpl_event):
        """
        处理鼠标左键释放事件，停止拖动 Marker Legend（Blitting 终极丝滑版）
        完美兼容 Normal / Max 模式 + AnnotationBbox / Text + Disable Refresh
        """
        
        # ===================== 1. 处理 Marker Legend 拖动结束 =====================
        if self.dragging_marker_legend:
            
            # 取消可能的节流任务（虽然 Blitting 版基本不用，但保险）
            if self._drag_update_id:
                self.root.after_cancel(self._drag_update_id)
                self._drag_update_id = None
            ...
            # ==================== Blitting 专用清理 ====================
            # 关键：清除背景缓存 + 拖动状态，防止内存泄漏或下次误用
            self._drag_blit_cache_valid = False
            self.drag_legend_ax = None
            self.drag_legend_artist = None
            # 可选：保留背景缓存供下次拖动更快（推荐保留，不清理）
            # self.drag_legend_bg.clear()  # 如果你想每次都重新缓存，可打开这行
            ...
            self.drag_canvas = None
            self.drag_param = None  # Normal 模式用的 param 也要清

            # ==================== Max 模式 Toolbar 防冲突 ====================
            if self.display_mode.get() == "Max" and self.max_toolbar:
                if getattr(self.max_toolbar, '_active', None):
                    try:
            ...
            # ==================== 最终落盘：只刷新一次（关键！）===================
            # 因为拖动过程中我们只用了 Blitting（没重绘数据线），所以必须主动刷新一次
            # 否则新位置不会写入 self.marker_pos_configs，也不会被保存/导出
            def _final_save_position():
                if self.freeze_refresh_var.get():
                    # 禁用刷新：只更新 Marker 位置，保留当前缩放/平移状态
                    self._safe_refresh_markers(reset_limits=False)
                else:
                    # 正常模式：完整重绘（确保位置永久写入配置）
                    if self.display_mode.get() == "Normal":
                        self.update_plots()
                    else:
                        self.plot_combined()  # Max 模式用专用函数

            # 使用 after_idle 确保所有事件处理完再刷新（最稳定）
            self.root.after_idle(_final_save_position)
        ...
        # ===================== 2. 非 Marker Legend 拖动时的保险处理 =====================
        # （比如用户在 Max 模式下用鼠标中键/右键释放 Pan/Zoom）
        if self.display_mode.get() == "Max" and self.max_toolbar:
            if getattr(self.max_toolbar, '_active', None):
                try:
                    self.max_toolbar.release_pan(mpl_event)
    ...
    def delete_marker_on_right_click(self, mpl_event, param=None):
        ...
        Y_TOLERANCE_BASE = {"Magnitude (dB)": 0.5, "Phase (deg)": 15.0, "Wrapped Phase (deg)": 5.0, "Group Delay (ns)": 5.0}.get(plot_type, 1.0)
        try:
    ...
    def on_display_mode_change(self, *args):
        ...
        # =========================================================
        # 【关键修复1】：在任何模式切换前，强制断开所有拖动事件 + 重置 Blitting 缓存
        # 这能彻底解决“第一次从 Normal 切到 Max 时拖动事件未连接”的根因
        # =========================================================
        self._disconnect_all_drag_events()          # 断开 Normal 和 Max 的所有拖动 CID
        self._drag_blit_cache_valid = False         # 强制下次绘制时重建背景缓存
        self.drag_legend_bg.clear()                 # 清空 Blitting 背景缓存

        # ----------------------------------------------------
        # 1. Show Params 重建 (必须在 Y 轴 UI 切换前)
        # ----------------------------------------------------
        if hasattr(self, "cb_frame"):
            self.cb_frame.pack(fill="x", padx=5, pady=(2, 8))
        self.rebuild_show_params()  # 更新 self.params 和复选框
        ...
        if mode == "Max":
            ...
            # 【关键修复2】：Max 模式绘图完成后，强制重新连接拖动事件
            # 即使 plot_combined 内部已经连接，这里再保一次险（防止被 Blitting 跳过）
            self._connect_max_drag_events()
        else:
            self.update_plots()           # Normal 模式绘图
            # Normal 模式会在 update_plots 内部自行处理事件连接，无需额外操作
        # ----------------------------------------------------


    def update_marker_controls_visibility(self):
        mode = self.display_mode.get()
        for plot_type in ["Magnitude (dB)", "Phase (deg)", "Wrapped Phase (deg)", "Group Delay (ns)"]:
            if plot_type not in self.data:
                continue
            for param in self.params:
                ui_refs = self.data[plot_type]["ui_refs"].get(param, {})
    ...
    def enter_max_mode(self):
        ...
        # --- 创建/显示 Max 模式 Frame 和 Canvas 逻辑 (保持不变) ---
        created = False
        ...
        if not getattr(self, "max_frame", None):
            ...
            if not getattr(self, "max_toolbar", None):
                ...
                try:
                    ...
                    pass
        # --- 创建/显示 Max 模式 Frame 和 Canvas 逻辑 结束 ---
        ...
        # Now bind events robustly to the concrete bind_target (保持事件绑定逻辑)
        ...
        # =========================================================
        # 【核心优化 V7】使用 50ms 延迟执行清除和重绘
        # 确保 Tkinter 布局完成后再绘图
        # =========================================================
        self.max_mode_active = True 
        
        # 使用 50ms 延迟执行绘图，让 Tkinter 有机会完成 Max 模式的 Frame 布局和尺寸确定
        if hasattr(self, 'root'):
            # 将延迟从 0 增加到 50 毫秒
            self.root.after(50, self._delayed_max_plot_refresh) 
            
        #------------------------------------

    def _delayed_max_plot_refresh(self):
        """
        Max 模式下延迟执行的绘图和刷新逻辑。
        目标是强制清除 Max Axes 上的旧内容，然后调用高层级的 update_plots。
        """
        # 确保 Max 模式已激活
        if not self.max_mode_active:
            return
            
        try:
            if getattr(self, "max_ax", None) and getattr(self, "max_canvas", None):
                
                # --- 1. 强制清除 Axes ---
                # 相比于逐个移除 line，简单清空 Axes 通常更可靠且更符合 Matplotlib 的重绘流程。
                # 这会清除所有 Lines, Text, Patch 等对象。
                self.max_ax.clear() 
                
                # --- 2. 调用高级绘图函数 ---
                # 假设 self.update_plots() 是最高级的绘图入口，并且它会根据 
                # self.max_mode_active 状态来调用 plot_combined 并绘制可见曲线。
                if hasattr(self, "update_plots"):
                    self.update_plots() 
                
                # --- 3. 强制 Canvas 绘制 ---
                # 这是将内存中的新图形推送到屏幕的关键步骤
                self.max_canvas.draw() 
                
                # 确保 Tkinter 刷新 (如果 draw() 不够，可以帮助解决布局同步问题)
                if hasattr(self, 'root'): 
                    self.root.update_idletasks()
                    
        except Exception as e:
            # 如果需要调试，可以在这里打印异常
            # print(f"Delayed Max Plot Refresh Error: {e}") 
            pass
    ...
    def update_plots(self):
        ...
        is_refresh_disabled = getattr(self, 'freeze_refresh_var', None).get() if hasattr(self, 'freeze_refresh_var') else False
        # ==================================================
        # Normal 模式
        # ==================================================
        if mode == "Normal":
            ...
            # ==================== 【最小化核心修复：动态补全 y_configs】 ====================
            # 确保当前 Port Pair 的所有 S 参数在 y_configs 中都有默认配置（解决 S23/S32 等新参数无 Y 轴设置的问题）
            if plot_type not in self.y_configs:
                self.y_configs[plot_type] = {}
            ...
            default_ymax = "0"
            if plot_type == "Phase (deg)":
                default_ymin, default_ymax = "-180", "180"
            elif plot_type == "Wrapped Phase (deg)":
                default_ymin, default_ymax = "-180", "180"
            elif plot_type == "Group Delay (ns)":
                default_ymin, default_ymax = "0", "10"

            for param in normal_params:
                if param not in self.y_configs[plot_type]:
                    self.y_configs[plot_type][param] = {
                        "mode": tk.StringVar(value="Default"),
                        "min":  tk.StringVar(value=default_ymin),
                        "max": tk.StringVar(value=default_ymax)
                    }
            # ==========================================================================
            ...
            num_selected = len(selected_params) # 获取选中的图表数量
            # 【核心修改区域开始】: 动态调整 Figure 尺寸
            for param in selected_params:
                ...
                if param not in self.plot_configs:
                    ...
                    if num_selected == 1:
                        ...
                        # 4个图表：2x2
                        fig_w, fig_h = 4, 3
                    fig = plt.Figure(figsize=(fig_w, fig_h), dpi=self.actual_dpi)
                    # -------------------------------------------------------------
                   
                    ax = fig.add_subplot(111)
                    fig.tight_layout(pad=1.0)
                   
                    frame_fg = {"S11": "blue", "S12": "green", "S21": "red", "S22": "purple"}.get(param, "black")
                    frame = tk.LabelFrame(
                        self.charts_frame,
                        text=" ",
                ...
                if has_data:
                    ...
                    # ==================== 【新增：Default 模式下自动调整 Y 轴范围】 ====================
                    y_mode = self.y_configs.get(plot_type, {}).get(param, {}).get("mode", tk.StringVar(value="Default")).get()
                    if y_mode == "Default" and plot_type == "Magnitude (dB)":
                        import math
                        all_y = []
                        for dataset in self.datasets:
                            s_key = param.lower()  # 如 's23'
                            if s_key in dataset['s_data']:
                                s_data = dataset['s_data'][s_key]
                                y_vals = 20 * np.log10(np.abs(s_data) + 1e-20)  # 防止 log(0)
                                all_y.extend(y_vals)

                        if all_y:
                            data_min = np.min(all_y)
                            data_max = np.max(all_y)

                            # 自动圆整：向下留裕量（步长5或10），确保低值不被截断
                            step = 5 if abs(data_min) < 100 else 10
                            rounded_min = math.floor(data_min / step) * step - step  # 如 -75.546 → -80
                            rounded_max = math.ceil(data_max / step) * step + step    # 上方留裕量

                            ax.set_ylim(rounded_min, rounded_max)

                            # 可选：同步更新 UI 配置（防止下次仍用旧值）
                            if param in self.y_configs.get(plot_type, {}):
                                self.y_configs[plot_type][param]["min"].set(str(rounded_min))
                                self.y_configs[plot_type][param]["max"].set(str(rounded_max))
                    # ==============================================================================
                else:
                    ax.clear()
    ...
    def plot_combined(self, visible_params=None):
        ...
        y_unit = {"Magnitude (dB)": "dB", "Phase (deg)": "deg", "Wrapped Phase (deg)": "deg", "Group Delay (ns)": "ns"}.get(plot_type, "")
        
        # [新增] Max模式下 Y 轴标签隐藏控制
        ylabel_text = f"{plot_type.split('(')[0].strip()} ({y_unit})"

        hide_ylabel = False
        if hasattr(self, 'hide_ytitle_var'):
            try:
                # 尝试安全地获取 Tkinter 变量的值
                # 注意：如果 SAFE_BOOL 或类似的安全函数在类中已定义，建议使用它
                if isinstance(self.hide_ytitle_var, (tk.BooleanVar, tk.IntVar)):
                    hide_ylabel = self.hide_ytitle_var.get()
                elif hasattr(self, 'SAFE_BOOL'):
                    hide_ylabel = self.SAFE_BOOL(self.hide_ytitle_var)
            except:
                pass
                    ...
                    try:
                        ...
                        if min_f < max_f:
                            ...
                            if abs(start_mhz - stop_mhz) < 1e-6: # 检查是否为水平线（频率相同）
                                ...
                                # [新增] 获取 Limit Line 颜色
                                limit_color = self.get_limit_line_color(plot_type, p)
                            # --- Upper Limit Line 绘制 ---
                            if ltype in ["Max", "Band", "Upper Only", "Upper/Lower", "Upper Limit"]:
                                ax.plot(
                            ...
                            if ltype in ["Min", "Band", "Lower Only", "Upper/Lower", "Lower Limit"]:
                                ax.plot(
                                    [x_line_start, x_line_stop],
                                    [y_line_start, y_line_stop], # FIX: 使用插值后的 Y 值
                                    color=limit_color, # 假设 Lower Limit 用蓝色虚线，如果需要绿色，请改成 'green'
                                    linestyle='--',
                                    linewidth=1.0,
                                    zorder=4
                                )
        ...
        if self.max_fig:
            ...
            self.max_fig.subplots_adjust(top=0.925, bottom=0.08, left=0.055, right=0.98)
    ...
    def _get_y_data_for_limit_calc(self, dataset, param, plot_type):
        """
        Get Y data array for auto Y-scaling / limit calculation.
        This function must NEVER depend on outer-scope variables.
        """

        # ===== ① 统一获取 S 参数 =====
        s_data = dataset.get('s_data', {}).get(param.lower())
    ...
    def _calculate_friendly_y_limits(self, y_min_data, y_max_data, target_steps=8, param_name=None):
        ...
        计算友好的 Y 轴限制和步长。
        核心逻辑：
        1. 增加候选步长密度（包含 2.5, 4 等）。
        2. 引入 TOLERANCE 容错，防止数据微超步长时（如 10.06）导致刻度跨度过大。
        3. 针对 dB 图像优化负值上限，确保顶部留白美观。
        """
        # --- 1. 边界情况处理 (数据点全部重合或范围极小) ---
        if abs(y_max_data - y_min_data) < 1e-9:
            y_min = y_min_data - 5.0
            y_max = y_min_data + 5.0
            step = 2.0
            if abs(y_min_data) < 1.0:
                y_min, y_max, step = -2.0, 2.0, 1.0
        ...
        # --- 3. 定义高密度候选步长 (方法 B 优化) ---
        # 增加 2.5 和 4 档位，使步长在 1, 2, 2.5, 4, 5, 10 之间平滑过渡
        if param_name and param_name.upper() in ["S11", "S22"]:
            candidates = [10*p, 5*p, 2.5*p, 2*p, 1*p]
        elif param_name and param_name.upper() in ["S21", "S12"]:
            candidates = [10*p, 5*p, 4*p, 2.5*p, 2*p, 1*p]
        else:
            candidates = [10*p, 5*p, 2.5*p, 2*p, 1*p]
        
        # --- 4. 步长筛选 (引入容错系数) ---
        # TOLERANCE = 0.8 意味着：如果 base_step 是 10.3，我们依然允许使用 10 的步长
        TOLERANCE = 0.8
        friendly_step = candidates[0]
        for c in candidates:
            if c >= base_step * TOLERANCE:
                friendly_step = c
            else:
                break
        step = friendly_step
        ...
        # --- 5. 确定 Y 轴范围 ---
        # BUFFER_DISTANCE 确保曲线顶部不压线
        BUFFER_DISTANCE = 2.0 
        target_max = y_max_data + BUFFER_DISTANCE
        
        # 计算下限：使用 floor 向下取整到步长的整数倍
        ...
        if y_max_data >= 0:
            ...
            # 当数据全为负（dB 图）
            if target_max <= 0.0:
                # 如果加上缓冲后仍小于0值，通常 S 参数图上限设为 0dB 最美观
                y_max = 0.0
            else:
                # 如果数据非常靠近 0dB，缓冲后超过了 0，则向上取整
                y_max_coarse = np.ceil(target_max / step) * step
                # 限制上限跳变，避免出现 10dB/20dB 这种不必要的正值空间
                if target_max <= 2.0:
                    y_max = 2.0
                elif target_max <= 5.0:
                    y_max = 5.0
    ...
    def plot_parameter_output(self, ax, fig, param, plot_type):
        ...
        # y label
        y_unit = ""
        ylabel_text = "" # 新增变量存储 Y 轴标签文本
        if plot_type == "Magnitude (dB)":
            y_unit = "dB"
            ylabel_text = "Magnitude (dB)"
        elif plot_type == "Phase (deg)":
            y_unit = "deg"
            ylabel_text = "Phase (deg)"
        elif plot_type == "Wrapped Phase (deg)":
            y_unit = "deg"
            ylabel_text = "Wrapped Phase (deg)"
        elif plot_type == "Group Delay (ns)":
            y_unit = "ns"
            ylabel_text = "Group Delay (ns)"
        
        # [新增] 根据 self.hide_ytitle_var 决定最终标签是否显示
        hide_ylabel = False
        if hasattr(self, 'hide_ytitle_var'):
            try:
                # 使用 SAFE_BOOL 检查控制变量（假设是 Tkinter Var）
                hide_ylabel = SAFE_BOOL(self.hide_ytitle_var)
            except:
                pass
        ...
        for dataset in self.datasets:
            ...
            color = self.custom_id_colors.get(data_id, COLOR_CYCLE[(data_id - 1) % len(COLOR_CYCLE)])
        ...
        # y axis
        # ========== Y Axis 智能分组逻辑 (针对 Normal 模式下的 Port Pair 优化) ==========
        y_mode = self.y_configs[plot_type][param]["mode"].get()
        if y_mode != "Custom" and all_y_values:
            # 只有在 Magnitude (dB) 模式下才进行参数分组缩放
            if plot_type == "Magnitude (dB)":
                p_upper = param.upper()
                # 提取数字，例如 S13 -> ['1', '3']
                digits = [c for c in p_upper if c.isdigit()]
                
                if len(digits) >= 2:
                    # 获取当前 Port Pair 的具体端口号，例如 p1=1, p2=3
                    # 注意：此时 self.params 正是 ['S11', 'S13', 'S31', 'S33']
                    if digits[0] == digits[1]:
                        # 反射参数分组：提取当前 4 个参数中的对角线项 (如 S11, S33)
                        group_params = [p for p in self.params if len(p) >= 3 and p[1] == p[2]]
                    else:
                        # 传输参数分组：提取当前 4 个参数中的交叉项 (如 S13, S31)
                        group_params = [p for p in self.params if len(p) >= 3 and p[1] != p[2]]
                else:
                    group_params = [param]
            else:
                # Phase, Group Delay 等模式下，每个参数独立 Auto Scaling
                group_params = [param]

            # 收集组内数据（仅从当前选中的 Port Pair 相关的 4 个参数中收集）
            grouped = []
            for dataset in self.datasets:
                for p in group_params:
                    # 确保参数在 dataset 的 s_data 中存在（比如 s_data 里有 s13）
                    if p.lower() in dataset['s_data']:
                        y_d = self._get_y_data_for_limit_calc(dataset, p, plot_type)
                        if len(y_d) > 0: 
                            grouped.append(y_d)

            if grouped:
                y_all = np.concatenate(grouped)
                y_min, y_max = np.min(y_all), np.max(y_all)
                # 调用美化刻度的函数
                y_min, y_max, y_step = self._calculate_friendly_y_limits(y_min, y_max, param_name=param)                
                ax.set_ylim(y_min, y_max)
                ax.yaxis.set_major_locator(MultipleLocator(y_step))
                ax.yaxis.set_minor_locator(MultipleLocator(y_step/2))
                ax.grid(True, which='minor', linestyle=':', linewidth=0.5, color='gray', alpha=0.5)
            ...
            try:
                mn = float(self.y_configs[plot_type][param]["min"].get())
                mx = float(self.y_configs[plot_type][param]["max"].get())
                ax.set_ylim(mn, mx)
                ax.yaxis.set_minor_locator(AutoMinorLocator(2))
            except:
                pass

        # ========== Limit lines & Markers Logic (保持原样) ==========
        ...
        if plot_type in self.data and self.datasets and all_freq_values:
            ...
            for line in self.data[plot_type]["limit_lines"][param]:
                try:
                    ...
                    # [新增] 获取 Limit Line 颜色
                    limit_color = self.get_limit_line_color(plot_type, param)
                    ...
                    if ltype in ["Min", "Band", "Lower Only", "Upper/Lower", "Lower Limit"]:
                        ax.plot(
                            [x_line_start, x_line_stop], 
                            [y_line_start, y_line_stop], # FIX: 使用插值后的 Y 值
                            color=limit_color, # 请根据您的规范调整 Lower Limit 的颜色和线型
                            linestyle='--', 
                            linewidth=1.0, 
                            zorder=4,
                            # 存储 artist 引用以备后用 (可选)
        ...
        if plot_type in self.data and self.datasets:
            ...
            for mark in self.data[plot_type]["marks"][param]:
                ...
                try:
                    # ------------ 全部变量统一安全读取 ------------
                    display_status = SAFE(mark.get("display_status"), "Display")
                    is_search = SAFE_BOOL(mark.get("is_search"))
                    is_avg    = SAFE_BOOL(mark.get("is_avg"))

                    start_str = SAFE(mark.get("start"))
                    stop_str  = SAFE(mark.get("stop"))
                    unit      = SAFE(mark.get("unit"), "MHz")
                    freq_str  = SAFE(mark.get("freq"), "")

                    selected_data_id = SAFE(mark.get("data_id"))
                    dataset = next((d for d in self.datasets if str(d["id"]) == str(selected_data_id)), None)

                    mark_id = SAFE(mark.get("id"), "?")

                    # ======= dataset 不存在 → 必须立即 continue（防止 0MHz）=======
                    if not dataset:
                        if is_search and display_status == "Display":
                            x_display = f"{start_str}-{stop_str}{unit}"
                            legend_txt=f"{mark_id} ({param} ID {selected_data_id}) @{x_display}, N/A (No dataset)"
                            visible_marker_info_list.append((mark_id, str(selected_data_id), legend_txt))
                        continue
                    ...
                    if is_search:
                        ...
                        try:
                            start_val=float(start_str)
                            stop_val=float(stop_str)
                            mul={"GHz":1e9,"MHz":1e6,"KHz":1e3}.get(unit,1)
                            f_min,f_max= sorted([start_val*mul, stop_val*mul])
                            ...
                            if f_max < min_f_hz or f_min > max_f_hz:
                                ...
                                if not np.any(mask):
                                    ...
                                    search_type=SAFE(mark.get("search_type"))
                                    if search_type=="Max Value":
                                        idx=np.argmax(y_m)
                                        precise=freq_m[idx]
                                        y_pt=y_m[idx]
                                        ...
                                        # custom search
                                        custom_val=float(SAFE(mark.get("custom_value"),"0"))
                                        match_type=SAFE(mark.get("match_type"),"First Match")
                                        precise=self._find_custom_match_freq_hz(custom_val,freq_m,y_m,match_type)
                                        if precise is None:
                                            idx=np.argmin(np.abs(y_m-custom_val))
                                            precise=freq_m[idx]
                                            y_pt=y_m[idx]
                                        else:
                                            val_i=self.safe_interp(precise,freq,data_array)
                                            y_pt=val_i if val_i is not None else y_m[np.argmin(np.abs(freq_m-precise))]

                                    precise_mhz=precise/1e6
                                    x_display=f"{start_str}-{stop_str}{unit}, {precise_mhz:.3f} MHz"
                                    y_str=f"{y_pt:.3f} {y_unit}"
                                    x_pt_plot=np.clip(precise_mhz,x_min_mhz,x_max_mhz)
                                    plot_marker_point=True
                    ...
                    # ================================================================
                        ...
                        try:
                            ...
                            f_min,f_max=sorted([start_val*mul, stop_val*mul])
                            mask=(freq>=f_min)&(freq<=f_max)
                            avg_val=np.mean(data_array[mask]) if np.any(mask) else None

                            x_display=f"{start_str}-{stop_str}{unit}"
                            custom_name=self.custom_id_names.get(str(selected_data_id),f"ID {selected_data_id}")

                            y_str="N/A" if avg_val is None else f"{avg_val:.3f} {y_unit}"
                            legend_txt=f"{mark_id} ({param} {custom_name}) @{x_display}, {y_str}"
                            visible_marker_info_list.append((mark_id,str(selected_data_id),legend_txt))
                        except:
                            pass

                        continue  # 不允许落入 normal marker

                    # ================================================================
                    #                        ★ Normal Marker ★（修复0MHz BUG）
                    # ================================================================
                    else:
                        # freq="" → 无效，不允许 =0
                        if freq_str.strip()=="":
                        ...
                        target_hz=self._get_marker_freq_hz(mark)
                        if target_hz is None or target_hz <= 0:
                            continue

                        x_display=f"{freq_str} {unit}"

                        if min_f_hz <= target_hz <= max_f_hz:
                            val=self.safe_interp(target_hz,freq,data_array)
                            if val is not None:
                                y_pt=float(val)
                                x_pt_plot=np.clip(target_hz/1e6,x_min_mhz,x_max_mhz)
                                y_str=f"{y_pt:.3f} {y_unit}"
                                plot_marker_point=True
                            else:
                                y_str="N/A"
    ...
    def _reindex_markers_and_refresh_ui(self, plot_type, param):
        ...
        if search_marker_list_frame:
            ...
            for mark in search_marks:
                if "frame" in mark and mark["frame"]:
                    mark["frame"].destroy()
                mark["id"] = f"P{search_count}"  # 搜索标记前缀
                search_count += 1
                # 假设此方法存在
    ...
    def reset_application(self):
        ...
        if hasattr(self, 'display_vars'):
            self.display_vars.clear()  # 一行就够了！最简洁有效
        # >>>>>>>>>> 结束 <<<<<<<<<<
        ...
        # Disable hide_ytitle_var (default)
        if hasattr(self, 'hide_ytitle_var'):
            self.hide_ytitle_var.set(False)

        # [新增] 重置 Feature Controls 可折叠状态为展开 (Expanded=True)
        # 检查它是否是折叠状态 (False)
        if hasattr(self, 'feature_controls_expanded') and not self.feature_controls_expanded.get():
            # 调用 toggle 函数，将其从折叠状态切换到展开状态
            if hasattr(self, 'feature_controls_toggle'):
                self.feature_controls_toggle() 
        ...
        # 1. FIX: 暂时移除 on_port_pair_change 的监听，避免 display_mode 重置后 Port Pair 变量的设置导致二次 UI 重建
        ...
        # 2. 强制设置 Port Pair 和 S 参数到默认值 (满足用户要求)
        self.port_selection_var.set("1-2") # 确保 Port Pair 回到 1-2
        self.normal_mode_ports = (1, 2)
        self.default_params = ["S11", "S12", "S21", "S22"] # 确保 S 参数回到 S11, S12, S21, S22
        self.params = self.default_params.copy()

        # 3. 确保 S11, S12, S21, S22 的复选框状态是 True
        for p in self.default_params:
            if p not in self.show_param_vars:
                self.show_param_vars[p] = tk.BooleanVar(value=True)
            self.show_param_vars[p].set(True)

        # 4. 重置核心模式变量 (此调用会触发 on_display_mode_change -> rebuild_show_params，完成单次 UI 重建)
        ...
        # ------------------ 🔥 FIX: 重新添加 on_port_pair_change 的监听 🔥 ------------------
        # 必须重新添加 trace，确保后续正常的 Port Pair 切换可以工作
        # 使用新的 ID 覆盖旧 ID (即使 trace_remove 失败，重新添加也可以)
        self._port_pair_change_trace_id = self.port_selection_var.trace_add("write", self.on_port_pair_change)
        # ------------------------------------------------------------------------------------
...
# ------------------------------------------------------------------
    ...
    def _show_load_dialog(self):
        ...
        dialog.title("Select Data Source")
    ...
    def load_snp(self):
        ...
        for file_path in file_paths:
            try:
                ...
                # === 关键：立即为这个 dataset 创建显示状态变量，默认显示 ===
                self.display_vars[new_dataset['id']] = tk.BooleanVar(value=True)
                # ===========================================================                 
                self.next_dataset_id += 1
                loaded_count += 1
    ...
    def _handle_chart_output(self, copy=False):
        out_fig = None # 初始化 out_fig

        try:
            # ==================== Max 模式 ====================
            if self.display_mode.get() == "Max":

                if not hasattr(self, 'max_fig') or self.max_fig is None:
                    messagebox.showwarning("No Plot", "Max mode plot not available. Please load data first.")
                    return
                ...
                # 添加标题
                title_line1 = self.title_var.get()
                target_fig.text(0.5, 0.975, title_line1, ha='center', va='top',
                                 fontsize=14, fontweight="bold")

                # Logo 尺寸切换（你原来的逻辑）120x48
                current_height = target_fig.bbox.height
                ...
                self._draw_report_logo(target_fig,
                                       y_offset=logo_y_offset,
                ...
                if copy:
                    buf = io.BytesIO()
                    target_fig.savefig(buf, format='png', dpi=300, bbox_inches='tight')
                    buf.seek(0)
                    img = Image.open(buf)
                    ...
                    if not f:
                        ...
                        if f.lower().endswith('.pdf'):
                            target_fig.savefig(f, format='pdf', dpi=300, bbox_inches='tight',
                                               metadata={'Title': title_line1})
                            messagebox.showinfo("Saved", f"Saved as PDF:\n{f}")
                        else:
                            target_fig.savefig(f, format='png', dpi=300, bbox_inches='tight')
                            messagebox.showinfo("Saved", f"Saved as PNG:\n{f}")
            ...
            # ==================== Normal 模式 ====================
                ...
                out_fig = None
                original_datasets = self.datasets # 备份原始数据，包含所有文件

                try:
                    # 过滤出所有可见的 datasets
                    visible_datasets = [d for d in original_datasets if d.get('is_displayed', True)]

                    # 核心修复：临时将实例的 datasets 替换为可见列表
                    self.datasets = visible_datasets 
                    ...
                    if not selected_params:
                        messagebox.showwarning("Warning", "No S-parameters selected for plotting.")
                        return 

                    # 确定是否需要绘制顶部 ID 颜色示例，现在基于可见文件数量
                    show_top_legend = len(self.datasets) >= 2 
                    
                    # 2. 创建最终输出 Figure
                    OUT_W_INCH, OUT_H_INCH = 12, 8
                    out_fig = plt.figure(figsize=(OUT_W_INCH, OUT_H_INCH)) 
                    ...
                    gs = GridSpec(nrows=2, ncols=2, figure=out_fig, 
                                  hspace=0.25, wspace=0.15, 
                                  top=gs_top, bottom=0.08, left=0.05, right=0.98) 

                    # 4. 在新的 Axes 中重新绘制内容
                    for param, cfg_cfg in layout_config.items():
                        row = cfg_cfg.get('row', 0)
                        col = cfg_cfg.get('col', 0)
                        rowspan = cfg_cfg.get('rowspan', 1)
                        colspan = cfg_cfg.get('colspan', 1)

                        out_ax = out_fig.add_subplot(gs[row:row+rowspan, col:col+colspan])
                        # 调用 self.plot_parameter_output，它现在使用临时的 self.datasets (仅包含可见文件)
                        self.plot_parameter_output(out_ax, out_fig, param, self.plot_type.get())

                    # 5. 添加全局标题和条件 ID 线/图例
                    title_line1 = self.title_var.get()
                    
                    # 标题位置固定在高位
                    out_fig.suptitle(title_line1, y=0.99, fontsize=14, fontweight="bold")
                    
                    # --------------------- 关键逻辑 ---------------------
                    if show_top_legend:
                        # 仅对可见数据集绘制顶部的 ID 颜色示例
                        start_x = 0.25
                        spacing = 0.15
                        y_pos = 0.95 
                        
                        # 迭代 self.datasets (即 visible_datasets) 绘制图例
                        for i, d in enumerate(self.datasets): 
                            data_id = d['id']
                            name = self.custom_id_names.get(str(data_id), f"ID {data_id}")
                            color = self.custom_id_colors.get(data_id,
                                                            COLOR_CYCLE[(data_id - 1) % len(COLOR_CYCLE)])
                            x_pos = start_x + i * spacing
                            
                            out_fig.text(x_pos - 0.02, y_pos, name, ha='right', va='center',
                                         fontsize=10, color='black', fontweight="bold", transform=out_fig.transFigure)
                            out_fig.text(x_pos, y_pos, "—", ha='left', va='center',
                                         fontsize=14, color=color, fontweight="bold", transform=out_fig.transFigure)
                    # ----------------------------------------------------

                    # 6. 添加 Logo
                    self._draw_report_logo(out_fig,
                                           y_offset=10, x_offset=50, 
                                           logo_width=self.report_logo_width.get(),
                                           logo_height=self.report_logo_height.get())
                    ...
                    if copy:
                        ...
                        if not f:
                            plt.close(target_fig)
                            # 注意：return 之前 finally 块会执行，恢复 self.datasets
                            return

                        if f.lower().endswith('.pdf'):
                            target_fig.savefig(f, format='pdf', 
                                               metadata={'Title': title_line1}) 
                            messagebox.showinfo("Saved", f"Saved as PDF:\n{f}")
                        else:
                            target_fig.savefig(f, format='png', dpi=300, bbox_inches='tight')
                    ...
                    # 无论成功、失败还是提前返回，都必须恢复原始数据集
                    self.datasets = original_datasets
            # ------------------------------------------------------------------

        # 外部的 except 块，与 try 对齐
        except Exception as e:
            if out_fig and hasattr(out_fig, 'close'):
                plt.close(out_fig)
            messagebox.showerror("Export Failed", f"Error: {e}\n\nPlease make sure there is data plotted in the current mode.")
        #------------------------------------------------------------------
    ...
    def create_limit_mark_section(self, parent, plot_type, param):
        ...
        # 确保所有内部字典结构存在
        if "ui_refs" not in self.data[plot_type]: 
            self.data[plot_type]["ui_refs"] = {}
        if "limit_lines" not in self.data[plot_type]: 
            self.data[plot_type]["limit_lines"] = {}
        if "marks" not in self.data[plot_type]: 
            self.data[plot_type]["marks"] = {}
            
        # 确保 param 键在所有相关字典中存在
        if param not in self.data[plot_type]["ui_refs"]: 
            self.data[plot_type]["ui_refs"][param] = {}
        if param not in self.data[plot_type]["limit_lines"]: 
            self.data[plot_type]["limit_lines"][param] = []
        if param not in self.data[plot_type]["marks"]: 
        ...
        def draw_limit_line_frame(line_data, container):
            ...
            # 绑定 stop_unit_var 的变化回调，让 start_unit_var 跟随
            def on_stop_unit_change(*args):
                start_unit_var.set(stop_unit_var.get()) # 同步单位
                
            stop_unit_var.trace_add("write", on_stop_unit_change) 
            ...
            def remove_and_update():
                ...
                # ❌ 移除 self.update_plots()，避免页面跳转
                # self.update_plots() 
                self.update_limit_color_button_visibility(plot_type, param)
                if hasattr(self, 'status_var'):
                    self.status_var.set(f"Limit Line removed for {param}. Click 'Refresh Plot' to update plot.")
        ...
        # 定义手动刷新绘图的函数
        def refresh_limit_plot():
            self.update_plots()
            self.update_limit_color_button_visibility(plot_type, param)
            if hasattr(self, 'status_var'):
                self.status_var.set(f"Limit Line plot refreshed for {param}.")
        ...
        def add_limit_and_draw():
            ...
            self.update_limit_color_button_visibility(plot_type, param)
            if hasattr(self, 'status_var'):
                self.status_var.set(f"Limit Line added for {param}. Click 'Refresh Plot' to update plot.")

            
        tk.Button(limit_control_frame, text="Add Limit Line", bg="#2ecc71", fg="white", command=add_limit_and_draw, width=12).pack(side="left", padx=5)
        
        # ==================== [Limit Line 颜色控制] ====================
        # 获取当前 Limit Line 颜色，用于初始化显示
        current_color = self.get_limit_line_color(plot_type, param)

        # 按钮容器：用于控制整体显示/隐藏，并将其打包到 limit_control_frame 中
        color_button_container = tk.Frame(limit_control_frame, bg="#f0f2f5")
        color_button_container.pack(side="left", padx=(20, 0)) # ✅ 关键：确保容器被打包

        # 1. 颜色显示框 (Label 模拟，显示当前颜色)
        ...
        color_display_label.pack(side="left", padx=(0, 5), pady=3)

        # 2. Custom Color 按钮
        custom_color_btn = tk.Button(
            color_button_container, 
            text="Custom Color", bg="#3498db", fg="white", 
            width=12,
            command=lambda p=param, pt=plot_type: self._on_custom_limit_color(pt, p)
        )
        custom_color_btn.pack(side="left", padx=(10, 5))

        # 3. Reset Color 按钮
        reset_color_btn = tk.Button(
            color_button_container, 
            text="Reset Color", bg="#e74c3c", fg="white", 
            width=12,
            command=lambda p=param, pt=plot_type: self._on_reset_limit_color(pt, p)
        )
        reset_color_btn.pack(side="left", padx=(10, 5))

        # 存储 UI 引用
        self.data[plot_type]["ui_refs"][param]["limit_color_container"] = color_button_container
        self.data[plot_type]["ui_refs"][param]["limit_color_display"] = color_display_label 
        ...
        # ✅ NameError 修复：确保 marker_control_frame 在使用前被定义
        ...
        # ⚠️ 关键 Bug 修复：更新 ui_refs 时使用键值赋值
        self.data[plot_type]["ui_refs"][param]["marker_list_frame"] = marker_list_frame
        self.data[plot_type]["ui_refs"][param]["canvas"] = canvas
        ...
        # 存储 search_marker_list_frame 到 ui_refs
        ...
        tk.Label(avg_container, text="Show Average Value",
                  font=("sans-serif", 11, "bold"),
        ...
        tk.Button(avg_control_frame, text="Show Average",
                  bg="#2ecc71", fg="white",
    ...
    def create_data_information_tab(self):
        ...
        try:
            ...
            self.notebook.add(self.data_information_tab, text=" File & Settings ")
        ...
        if not hasattr(self, 'custom_id_outer'):
            ...
            tk.Button(input_frame, text="Apply", command=self.set_custom_id_name, bg="#3498db", fg="white", width=10).pack(side="left", padx=(10, 10))
        ...
        if not hasattr(self, 'report_logo_group'):
            ...
            # 选择按钮
            tk.Button(btn_frame, text="Choose Logo", bg="#3498db", fg="white", command=self._select_report_logo_file, width=12).pack(side="left", padx=(10, 10)) 
    ...
    # Logo图片
    def _draw_report_logo(self, fig, y_offset=10, x_offset=10, logo_width="120", logo_height="48"):
        """
        【Axes 方案】在图像左上角绘制 Logo，尺寸由 logo_width, logo_height 指定。
        x_offset/y_offset 现在解释为 Logo 距离左右/顶部的像素边距 (top/left margin)。
        支持负值（e.g., x_offset=-10 表示向左偏移）。
        """
        ...
        try:
            ...
            x_offset_px = x_offset  # 使用传入的 x_offset（像素单位）
            ...
            # 新增：钳位 left 到有效范围，防止溢出
            left = max(0.0, min(1.0 - logo_width_rel, left))
            bottom = max(0.0, min(1.0 - logo_height_rel, bottom))  # 顺带钳位 bottom（增强鲁棒性）
    ...
    def update_marker_position_ui(self):
        """
        【功能合并】Normal 模式：根据 Port Pair 动态重建 4 个标签页
                    Max 模式：仅绑定变量 + 控制 Custom 框显示
        """
        ...
        # 【关键修复1】记住当前选中的 tab 索引，重建后恢复（防止跳 tab！）
        try:
            current_tab_index = self.normal_position_notebook.index("current")
        except:
            current_tab_index = 0

        # 【关键修复2】彻底清除旧 tab 和控件字典
        for tab in self.normal_position_notebook.tabs():
            self.normal_position_notebook.forget(tab)
        self.normal_position_controls = {}  # 必须清空，防止残留 S11/S12 等 key
        ...
        for i, param in enumerate(display_params):
            ...
            combo = ttk.Combobox(inner, values=self.MARKER_POSITIONS, state="readonly", width=12)
            ...
            try:
                ...
                # 绑定下拉框选择事件（关键！之前漏了）
                combo.bind("<<ComboboxSelected>>", lambda e: update_custom_visibility())
                cfg["mode_var"].trace_add("write", lambda *_: update_custom_visibility())
        ...
        if not display_params:
            placeholder = tk.Frame(self.normal_position_notebook, bg="#f0f2f5")
            tk.Label(placeholder, text="No supported parameters", bg="#f0f2f5", fg="gray").pack(pady=20)
            self.normal_position_notebook.add(placeholder, text=" None ")

        # 【关键修复3】恢复之前选中的 tab（彻底杜绝跳 tab）
        try:
            if display_params:
                restore_index = min(current_tab_index, len(display_params) - 1)
                self.normal_position_notebook.select(restore_index)
        except:
            pass
    ...
    def toggle_dataset_display(self, data_id):
        ...
        # ========== 【新增：隐藏后自动回退 Port Pair 到 1-2（如果剩余文件不支持当前配置）】 ==========
        if not is_visible:  # 只在隐藏时检查（显示时无需干预）
            # 计算剩余显示数据集的最大端口数
            max_remaining_ports = 2  # 默认假设 2 端口
            for dataset in self.datasets:
                if dataset.get('is_displayed', True):  # 只看显示的
                    max_remaining_ports = max(max_remaining_ports, dataset.get('n_ports', 2))
            
            current_pair = self.port_selection_var.get()
            p1, p2 = map(int, current_pair.split('-'))
            current_max_port = max(p1, p2)
            
            # 如果当前 Port Pair 需要 > max_remaining_ports 的端口，自动回退到 1-2
            if current_max_port > max_remaining_ports:
                self.port_selection_var.set("1-2")
                self.status_var.set(f"Port Pair auto-reset to 1-2 (remaining files support up to {max_remaining_ports} ports). Refresh to apply.")
        # =================================================================
    ...
    def update_file_list_ui(self):
        ...
        # 3. 初始化必要的字典
        if not hasattr(self, 'custom_id_colors'): self.custom_id_colors = {}
        if not hasattr(self, 'display_vars'): self.display_vars = {}

        # 4. 遍历数据集构建 UI
        for dataset in self.datasets:
            data_id = dataset['id']

            # --- 关键：确保 BooleanVar 存在且唯一绑定 ID ---
            if data_id not in self.display_vars:
                initial_val = dataset.get('is_displayed', True)
                self.display_vars[data_id] = tk.BooleanVar(value=initial_val)
            ...
            tk.Button(btn_frame, text="🎨", width=3, font=("Segoe UI Symbol", 8),
                      command=lambda d_id=data_id: self.open_color_setting_dialog(d_id)).pack(side="left")
            
            tk.Button(btn_frame, text="↺", width=2, font=("Segoe UI Symbol", 9, "bold"),
                      fg="#d35400", relief="flat", bg="#f0f2f5",
                      command=lambda d_id=data_id: self.reset_custom_color(d_id)).pack(side="left", padx=(4, 0))

            # --- 中间：文件名 ---
            label = tk.Label(file_row_frame, text=f"ID {data_id} - {file_name_only}",
                            bg="#f0f2f5", fg=color, font=("sans-serif", 10, "bold"), anchor="w")
            label.pack(side="left", padx=5, fill="x", expand=True)
            self.create_tooltip(label, dataset['name'])

            # --- 右侧：操作按钮 (注意：先 pack 的在最右边) ---
            # 1. 删除按钮
            remove_btn = tk.Button(
                file_row_frame, text="X", bg="#e74c3c", fg="white",
                font=("sans-serif", 8, "bold"), width=2, relief="flat",
                command=lambda d_id=data_id: self.execute_full_remove(d_id) # 调用外部统一方法
            )
            ...
            display_check = tk.Checkbutton(
                file_row_frame, text="Display", variable=display_var,
                bg="#f0f2f5", activebackground="#f0f2f5",
                command=lambda d_id=data_id: self.toggle_dataset_display(d_id)
            )
            display_check.pack(side="right", padx=(5, 5))
            self.create_tooltip(display_check, "Show / Hide this file in plot")

    def execute_full_remove(self, data_id_to_remove):
        """彻底删除数据集及其关联的所有 UI 状态"""
        # 1. 物理移除数据
        self.remove_dataset(data_id_to_remove)
        
        # 2. 核心修复：立即清理状态变量，防止 ID 漂移导致勾选错乱
        if data_id_to_remove in self.display_vars:
            del self.display_vars[data_id_to_remove]

        if data_id_to_remove in self.custom_id_colors:
            del self.custom_id_colors[data_id_to_remove]

        # 3. 联动刷新所有 UI 组件
        self.update_file_list_ui()          # 刷新左侧列表
        self.update_limit_mark_params()      # 更新 S 参数列表逻辑
        self.rebuild_show_params()           # 重建右侧/下方 S 参数勾选区
        self.update_data_information_tab()   # 更新信息表格
    ...
    def open_color_setting_dialog(self, dataset_id):
        ...
        if mode == "Normal":
            ...
            tk.Button(win, text="Choose", width=10,
    ...
    def remove_dataset(self, data_id):
        # --- 核心修复：状态重映射准备 ---
        # 暂存旧的 custom_id_names 和 display_vars
        old_custom_id_names = self.custom_id_names
        # 必须暂存 display_vars 的值（True/False），因为 BooleanVar 对象在销毁 UI 时可能会重置
        old_display_states = {k: v.get() for k, v in self.display_vars.items()}
        
        # 1. 物理移除数据集
        self.datasets = [d for d in self.datasets if d['id'] != data_id]
        
        # 2. 重新编号剩余的数据集，并同步所有状态字典
        self.custom_id_names = {}
        # 注意：这里不清空 self.display_vars，我们将在 update_file_list_ui 中重新绑定
        new_display_vars = {} 
        ...
        for i, d in enumerate(self.datasets):
            ...
            # A. 重映射 custom_id_names
            if str(old_id) in old_custom_id_names:
                self.custom_id_names[str(new_id)] = old_custom_id_names[str(old_id)]
            
            # B. 重映射勾选状态 (维持 ID4 被勾选的状态到新的 ID3 上)
            if old_id in old_display_states:
                new_display_vars[new_id] = tk.BooleanVar(value=old_display_states[old_id])
            else:
                new_display_vars[new_id] = tk.BooleanVar(value=True)
                
        # 更新全局状态字典
        self.display_vars = new_display_vars
        self.next_dataset_id = len(self.datasets) + 1

        # ------------------------------------------------------------
        # 3. 同步刷新所有关联 UI (这部分非常重要)
        # ------------------------------------------------------------
        ...
        # 关键刷新链：必须按此顺序执行
        self.update_file_list_ui()        # 刷新左侧列表，重新渲染 Checkbox
        self.update_limit_mark_params()    # 更新标记点的 S 参数对应关系
        self.rebuild_show_params()         # 刷新右侧 S 参数勾选框（ID 变了，这里必须重画）
        self.update_data_information_tab() # 刷新信息选项卡
        self.on_plot_type_change()         # 重新绘图（确保图表同步）


    #------------------------------
    def on_plot_type_change(self, *args):
        plot_type = self.plot_type.get()
        is_supported = plot_type in ["Magnitude (dB)", "Phase (deg)", "Wrapped Phase (deg)", "Group Delay (ns)"]
        ...
        if hasattr(self, 'y_sub_notebook'):
            ...
            try:
                ...
                pass
        # 5. 【新增关键】更新 File & Settings 页的 Marker Legend Position 控件绑定
    ...
    def create_marker_position_config_ui(self):
        """在 File & Settings 页创建 Marker Legend Position 控制区（仅创建一次）"""
        ...
        # 【关键修改】不再使用 self.params（硬编码），改用当前 Port Pair 计算
        try:
            sel = self.port_selection_var.get()
            p1, p2 = map(int, sel.split('-'))
            initial_params = [f"S{p1}{p1}", f"S{p1}{p2}", f"S{p2}{p1}", f"S{p2}{p2}"]
        except Exception:
            initial_params = ["S11", "S12", "S21", "S22"]

        # 支持参数过滤（与 update 函数保持一致）
        supported_params = set()
        for dataset in self.datasets:
            n_ports = dataset.get('n_ports', 2)
            for r in range(1, n_ports + 1):
                for c in range(1, n_ports + 1):
                    supported_params.add(f"S{r}{c}")
        display_params = [p for p in initial_params if p in supported_params or p in initial_params[:2]]  # 至少保留前两个
        ...
        if not display_params:
            placeholder = tk.Frame(self.normal_position_notebook, bg="#f0f2f5")
            tk.Label(placeholder, text="Waiting for data...", bg="#f0f2f5", fg="gray").pack(pady=20)
            self.normal_position_notebook.add(placeholder, text=" None ")
        ...
        max_combo = ttk.Combobox(max_inner, values=self.MARKER_POSITIONS, state="readonly", width=12)
        ...
        self.normal_position_notebook.pack(fill="x", pady=(5, 0))
        self.max_position_frame.pack(fill="x", pady=(10, 0))
        ...
        def on_treeview_motion(event):
            ...
            if item:
                ...
                if col == "#2":
                    ...
                    if hasattr(self, 'status_var'):
                        self.status_var.set("File & Settings")
            else:
                if hasattr(self, 'status_var'):
                    self.status_var.set("File & Settings")
