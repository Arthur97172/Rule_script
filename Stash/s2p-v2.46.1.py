...
class SViewGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("S-View Created By Arthur Gu | V2.46.1")
    ...
    def add_marker_on_click_combined(self, mpl_event):
        ...
        if not self.disable_refresh_var.get():
            # Auto Refresh 处于 Enable 状态，执行完整刷新
            self.update_plots()
            self.status_var.set(f"Marker added at {x_mhz:.3f} MHz on combined view for visible params.")
        else:
            # Auto Refresh 处于 Disable 状态，仅刷新 Marker 显示 (黄色区域和 Marker 点)
            try:
                self._update_max_marker_display() # 调用轻量级函数
                self.status_var.set(f"Marker added. Auto Refresh is DISABLED. Click 'Refresh Plots' to update.")
            except Exception as e:
                # 如果轻量级更新失败，更新状态栏
                self.status_var.set(f"Marker added, but manual Marker update failed: {e}. Auto Refresh is DISABLED.")
        # -------------------------- 优化逻辑结束 --------------------------
    ...
    def plot_combined(self, redraw_full=True): 
        ...
        # 8. Limit lines drawing for Combined Plot - 确保限制线始终绘制
        if plot_type in self.data and self.datasets:
            for p in visible_params:
             ...
             for p in visible_params:
                 for mark in self.data[plot_type]["marks"].get(p, []):
                     try:
                         ...
                         # 仅收集在 Max 模式下绘制的 Marker 相关的 Artists
                         if hasattr(self, 'max_marker_artists'):
