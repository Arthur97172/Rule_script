...
class SViewGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("S-View Created By Arthur Gu | V2.46.23")
...
# 文件: s2p-test1.py (在 SViewGUI 类内，替换 _setup_unified_y_control_ui 方法)
    ...
    def plot_combined(self, redraw_full=True):
        ...
        # ----------------------------------------------------
        # 11. 【MAX 模式 Marker Legend 绘制 - 优化后逻辑】
        # ----------------------------------------------------
        ...
        if visible_marker_info_list:
            ...
            def max_mode_sort_key(info):
                # info = (Marker ID str, Parameter str, Full Legend Text, Data ID str)
                param_str = info[1]
                data_id_str = info[3]
            ...
            # --- Marker Legend 核心参数 (与 Normal Mode 保持一致) ---
            LINE_HEIGHT_NORM = 0.030 # 最终调整后的行高
            HORIZONTAL_PADDING_NORM = 0.005 
            W_RECT_NORM = 0.30 # 归一化宽度
            ...
            if mode == "Top Left": x_val, y_val, h_align, v_align = 0.02, 0.98, 'left', 'top'
                ...
                try:
                    x_val = float(pos_config["x_var"].get())
                    y_val = float(pos_config["y_var"].get())
                    # 假设 Custom 模式提供了 h_align, v_align 的变量
    ...
    def create_data_information_tab(self):
        ...
        if not hasattr(self, 'custom_id_outer'):
            ...
            #现在New Name最多6个字符
            tk.Entry(input_frame, textvariable=self.custom_name_var, width=10).pack(side="left", padx=5)
...
#自定义文件ID名称
    def set_custom_id_name(self):
        ...
        # --- [二次验证：长度限制检查] ---
        # 实时限制已经在 _limit_custom_name_length 中完成，此处作为二次验证
        if len(new_name) > 6:
             # 此处应该不会被触发，因为前端已限制
            messagebox.showerror("Error", "The custom name cannot exceed 6 characters.")
            # 也可以不返回，直接截断：new_name = new_name[:6]
            return
        # ----------------------------------
