...
# 文件: s2p-test1.py (在 SViewGUI 类内，替换 _setup_unified_y_control_ui 方法)
    ...
    def update_plots(self):
        ...
        if self.datasets:
            for dataset in self.datasets:
                ...
                legend_items[data_id] = {'label': f"{name} uses: {points} points", 'color': color}
    ...
    def load_s2p(self):
        ...
        from tkinter import filedialog # 确保 filedialog 可用

        # ------------------------------------------------------------
        # 1. Ask User for Loading Method (English Dialog)
        # ------------------------------------------------------------
        
        # Ask user for selection method: Yes = Folder, No = Files
        choice = messagebox.askyesno(
            "Select Loading Method",
            "Do you want to load all S2P files from a folder?\n\nSelect 'Yes' to choose a directory.\nSelect 'No' to choose single or multiple files.",
            icon='question'
        )

        file_paths = []

        if choice: # Select Folder (Yes)
            directory_path = filedialog.askdirectory(title="Select Folder containing S2P Files")
            if directory_path:
                for filename in os.listdir(directory_path):
                    # Ensure only .s2p files are selected
        ...
        for file_path in file_paths:
            try:
                # ... (File loading and parsing logic remains unchanged) ...
