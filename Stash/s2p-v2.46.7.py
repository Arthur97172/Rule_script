...
class SViewGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("S-View Created By Arthur Gu | V2.46.7")
...
# 文件: s2p-test1.py (在 SViewGUI 类内，替换 _setup_unified_y_control_ui 方法)
    ...
    def add_marker_on_click_combined(self, mpl_event):
        ...
        if not self.disable_refresh_var.get():
            ...
            try:
                # 必须调用 plot_combined 来刷新 Matplotlib Canvas，以显示 Marker 点和标签（M1/M2）。
                ...
                self.status_var.set(f"Marker added, but plot update failed: {e}. Auto Refresh is DISABLED.")
        # -------------------------- 刷新逻辑优化结束 -------------------------- 
    ...
    def add_marker_on_click_normal(self, event, param):
        ...
        current_marks = self.data[plot_type]["marks"].get(param, [])
        next_id_number = len(current_marks) + 1
        mark_id = f"M{next_id_number}"
        ...
        if not self.disable_refresh_var.get():
            ...
            # 【修复不立即显示的 Bug】：手动调用 plot_normal 刷新当前图表
            self.plot_normal(param) 
